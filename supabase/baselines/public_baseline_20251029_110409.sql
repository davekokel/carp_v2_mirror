...
\restrict F3OYKJDTbtTEaq1F1NnSY4x11s5byCltx3UxFJ8dgg3HkDhINAxPO52OjQQGxzB
...
--
-- Name: _fp_pair_key(uuid, uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public._fp_pair_key(_mom_tank uuid, _dad_tank uuid) RETURNS text
    LANGUAGE sql STABLE
    AS $$
  with m as (select v.fish_code from public.v_tanks v where v.tank_uuid = _mom_tank),
       d as (select v.fish_code from public.v_tanks v where v.tank_uuid = _dad_tank)
  select case
           when (select fish_code from m) <= (select fish_code from d)
             then (select fish_code from m) || '~' || (select fish_code from d)
           else (select fish_code from d) || '~' || (select fish_code from m)
         end;
$$;
...
CREATE TABLE public.cross_instances (
    ...
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    tank_pair_code text,
    CONSTRAINT chk_ci_tank_pair_code_fmt CHECK ((tank_pair_code ~ '^TP\(FP-[0-9]{5}\)-[1-9][0-9]*$'::text))
);
...
CREATE TABLE public.tank_pairs (
    ...
    fish_pair_code text,
    note text,
    CONSTRAINT chk_fish_pair_code_fmt CHECK ((fish_pair_code ~ '^FP-[0-9]{5}$'::text)),
    CONSTRAINT chk_tank_pair_code_fmt CHECK ((tank_pair_code ~ '^TP\(FP-[0-9]{5}\)-[1-9][0-9]*$'::text))
);
...
CREATE VIEW public.v_containers_crossing_candidates AS
 SELECT container_id,
  ...
  WHERE ((container_type = 'tank'::text) AND (COALESCE(status, ''::text) = ANY (ARRAY['active'::text, 'ready'::text, 'available'::text, 'idle'::text])) AND (COALESCE(live_count, 0) = 0));
...
--
-- Name: ix_ci_tank_pair_code; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX ix_ci_tank_pair_code ON public.cross_instances USING btree (tank_pair_code);
...
--
-- Name: ix_cross_instances_tank_pair_code; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX ix_cross_instances_tank_pair_code ON public.cross_instances USING btree (tank_pair_code);
...
--
-- Name: ix_tank_pairs_fish_pair_code; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX ix_tank_pairs_fish_pair_code ON public.tank_pairs USING btree (fish_pair_code);
...
--
-- Name: ux_tank_pairs_tank_pair_code; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_tank_pairs_tank_pair_code ON public.tank_pairs USING btree (tank_pair_code);
...
--
-- Name: tank_pairs trg_gen_tank_pair_code; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_gen_tank_pair_code BEFORE INSERT ON public.tank_pairs FOR EACH ROW EXECUTE FUNCTION public.gen_tank_pair_code();
...
\unrestrict F3OYKJDTbtTEaq1F1NnSY4x11s5byCltx3UxFJ8dgg3HkDhINAxPO52OjQQGxzB
