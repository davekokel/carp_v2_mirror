...
-- Clean rebuild of v_clutch_instances:
-- - keep the original shape/CTEs/names
-- - change the source of clutch_genotype_codes to the tokenized codes list (a;b;...)
-- - use the corrected codes in the lineage expressions
...
-- tokens of offspring codes (p_code) extracted from the pretty string
geno_codes AS (
  SELECT
    b.clutch_code,
...
-- ✅ semicolon-joined offspring codes (distinct, ordered)
geno_codes_join AS (
  SELECT
    g.clutch_code,
...
SELECT
  ...
  -- ✅ corrected: offspring genotype codes from tokens (a;b;...)
  ...
  -- Keep the original lineage column names but feed the corrected codes
  CASE
  ...
  CASE
    WHEN NULLIF(tf.clutch_treatments_fusions, '') IS NOT NULL
      THEN tf.clutch_treatments_fusions || ' > ' || COALESCE(gr.clutch_genotype_fusions, gcj.clutch_genotype_codes)
