...
create index if not exists idx_pf_plasmid    on public.plasmid_fusions(plasmid_code);
...
begin
  if not exists (select 1 from pg_trigger where tgname='trg_fluors_touch') then
    create trigger trg_fluors_touch before update on public.fluors  for each row execute function public.touch_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname='trg_tags_touch') then
    create trigger trg_tags_touch  before update on public.tags    for each row execute function public.touch_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname='trg_fusions_touch') then
    create trigger trg_fusions_touch before update on public.fusions for each row execute function public.touch_updated_at();
  end if;
end $$;
