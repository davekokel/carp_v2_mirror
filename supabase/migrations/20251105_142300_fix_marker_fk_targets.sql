...
ALTER TABLE IF EXISTS public.ft_proteins DROP CONSTRAINT IF EXISTS fk_ftproteins_fluor;
ALTER TABLE IF EXISTS public.ft_proteins DROP CONSTRAINT IF EXISTS fk_ftproteins_tag;
ALTER TABLE IF EXISTS public.ft_dyes     DROP CONSTRAINT IF EXISTS fk_ftdyes_dye;
...
BEGIN
  ...
  ELSIF to_regclass('public.fluor_names') IS NOT NULL THEN
    ...
    LIMIT 1;
    IF col IS NOT NULL THEN
      EXECUTE format('ALTER TABLE public.ft_proteins ADD CONSTRAINT fk_ftproteins_fluor FOREIGN KEY (fluor_code) REFERENCES public.fluor_names(%I)', col);
    END IF;
  END IF;
  ...
  ELSIF to_regclass('public.tag_names') IS NOT NULL THEN
    ...
    LIMIT 1;
    IF col IS NOT NULL THEN
      EXECUTE format('ALTER TABLE public.ft_proteins ADD CONSTRAINT fk_ftproteins_tag FOREIGN KEY (tag_code) REFERENCES public.tag_names(%I)', col);
    END IF;
  END IF;
...
-- Recreate marker views using the stable LU helpers and with created_at
DO $$
