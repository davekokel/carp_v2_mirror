...
SELECT
  ...
  string_agg(DISTINCT t.tx_gt_tg, '; ' ORDER BY t.tx_gt_tg)
    FILTER (WHERE t.tx_gt_tg IS NOT NULL) AS tx_gt_tg,
  string_agg(DISTINCT t.tx_gt_fluortag, '; ' ORDER BY t.tx_gt_fluortag)
    FILTER (WHERE t.tx_gt_fluortag IS NOT NULL) AS tx_gt_fluortag,
  string_agg(DISTINCT t.tx_gt_fluororganelle, '; ' ORDER BY t.tx_gt_fluororganelle)
    FILTER (WHERE t.tx_gt_fluororganelle IS NOT NULL) AS tx_gt_fluororganelle,
...
LEFT JOIN public.treatments tr
  ON tr.id = tc.treatment_id
