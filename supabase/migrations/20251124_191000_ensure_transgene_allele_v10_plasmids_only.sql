...
BEGIN
  ...
  -- Ensure transgene exists, using plasmids as the only construct source (Option D).
  SELECT 1
  INTO v_dummy
  ...
  IF NOT FOUND THEN
    ...
    ELSE
      -- Unknown basecode: not in transgenes, not in plasmids.
      -- Return no rows; caller may log and skip.
      RETURN;
    END IF;
  END IF;
  ...
  -- Try to reuse existing (base_code, nickname) combination
  ...
  -- Mint new allele_number via global sequence
