...
CREATE TABLE IF NOT EXISTS public.ft_injection_mix_sources (
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uq_ftmix_src') THEN
...
-- Protein markers (surrogate PK + uniq)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uq_ft_protein_marker') THEN
    CREATE UNIQUE INDEX uq_ft_protein_marker
      ON public.ft_protein_markers (ft_code, COALESCE(tag_code,'∅'), fluor_code);
  END IF;
END$$;

-- Dyes catalog if missing
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='uq_ft_dye_marker') THEN
    CREATE UNIQUE INDEX uq_ft_dye_marker
