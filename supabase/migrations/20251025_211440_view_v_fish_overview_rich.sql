CREATE VIEW public.v_fish_overview_rich AS
 WITH current_tanks AS (
         ...
         SELECT DISTINCT fta.fish_id,
            ...
            fta.allele_number,
            COALESCE(ta.allele_name, ta.allele_nickname, ('#'::text || (fta.allele_number)::text)) AS allele_label
         ...
         SELECT r.fish_id,
            ...
            ( SELECT string_agg(u.transgene_base_code, '; '::text ORDER BY u.transgene_base_code) AS string_agg
                   FROM ( SELECT DISTINCT geno_rows.fish_id,
                            geno_rows.transgene_base_code
                           FROM geno_rows
                          WHERE (geno_rows.fish_id = r.fish_id)) u) AS transgene_pretty_name,
            ( SELECT string_agg(((u.transgene_base_code || ':'::text) || u.allele_label), '; '::text ORDER BY u.transgene_base_code, u.allele_label) AS string_agg
                   FROM ( SELECT DISTINCT geno_rows.fish_id,
                            geno_rows.transgene_base_code,
                            geno_rows.allele_label
                           FROM geno_rows
                          WHERE (geno_rows.fish_id = r.fish_id)) u) AS genotype_rollup
           FROM geno_rows r
          GROUP BY r.fish_id
        ), counts AS (
         SELECT vf.fish_code,
