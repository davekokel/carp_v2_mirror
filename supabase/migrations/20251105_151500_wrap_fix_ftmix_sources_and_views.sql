...
ALTER TABLE public.ft_protein_markers
  ADD CONSTRAINT fk_ftpm_ft FOREIGN KEY (ft_code) REFERENCES public.fluorescent_treatments(ft_code) ON DELETE CASCADE;
ALTER TABLE public.ft_dye_markers
  ADD CONSTRAINT fk_ftdm_ft FOREIGN KEY (ft_code) REFERENCES public.fluorescent_treatments(ft_code) ON DELETE CASCADE;
...
BEGIN
  SELECT column_name INTO fish_pk FROM information_schema.columns
   WHERE table_schema='public' AND table_name='fish' AND column_name IN ('fish_uuid','id','uuid')
   ORDER BY CASE column_name WHEN 'fish_uuid' THEN 1 WHEN 'id' THEN 2 WHEN 'uuid' THEN 3 ELSE 9 END LIMIT 1;

  SELECT column_name INTO fish_code_col FROM information_schema.columns
   WHERE table_schema='public' AND table_name='fish' AND column_name IN ('fish_code','code','name')
   ORDER BY CASE column_name WHEN 'fish_code' THEN 1 WHEN 'code' THEN 2 WHEN 'name' THEN 3 ELSE 9 END LIMIT 1;

  SELECT column_name INTO jfish_col FROM information_schema.columns
   WHERE table_schema='public' AND table_name='join_fish_fluorescent_treatments' AND column_name IN ('fish_uuid','fish_id','fish_code')
   ORDER BY CASE column_name WHEN 'fish_uuid' THEN 1 WHEN 'fish_id' THEN 2 WHEN 'fish_code' THEN 3 ELSE 9 END LIMIT 1;

  SELECT column_name INTO jft_col FROM information_schema.columns
   WHERE table_schema='public' AND table_name='join_fish_fluorescent_treatments' AND column_name IN ('ft_code','fluorescent_treatment_code','treatment_code')
   ORDER BY CASE column_name WHEN 'ft_code' THEN 1 WHEN 'fluorescent_treatment_code' THEN 2 WHEN 'treatment_code' THEN 3 ELSE 9 END LIMIT 1;

  SELECT column_name INTO jcreated_col FROM information_schema.columns
   WHERE table_schema='public' AND table_name='join_fish_fluorescent_treatments' AND column_name IN ('created_at','linked_at','updated_at')
   ORDER BY CASE column_name WHEN 'created_at' THEN 1 WHEN 'linked_at' THEN 2 WHEN 'updated_at' THEN 3 ELSE 9 END LIMIT 1;
