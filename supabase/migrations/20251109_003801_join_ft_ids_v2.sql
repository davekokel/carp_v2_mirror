...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname='fk_join_ft_dyes_ft_id__treatments_fluorescent_id'
      AND conrelid='public.join_ft_dyes'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fk_join_ft_dyes_dye_id__dyes_id'
      AND conrelid='public.join_ft_dyes'::regclass
  ) THEN
    ALTER TABLE public.join_ft_dyes
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fk_join_ft_fusions_ft_id__treatments_fluorescent_id'
      AND conrelid='public.join_ft_fusions'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fk_join_ft_fusions_fusion_id__fusions_id'
      AND conrelid='public.join_ft_fusions'::regclass
  ) THEN
    ALTER TABLE public.join_ft_fusions
