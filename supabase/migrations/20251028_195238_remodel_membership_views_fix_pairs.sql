...
-- v_tanks: open memberships to active tanks only
...
-- v_tank_pairs: resolve codes via fish_pairs + fish; tanks via tank_pairs + tanks
CREATE VIEW public.v_tank_pairs AS
SELECT
  tp.tank_pair_code::text           AS tank_pair_code,
  fp.fish_pair_code::text           AS fish_pair_code,
  fm.fish_code::text                AS mom_fish_code,
  fd.fish_code::text                AS dad_fish_code,
...
-- v_cross_clutch_instances: unchanged schema; uses v_tank_pairs + v_fish_rich
CREATE VIEW public.v_cross_clutch_instances AS
WITH mom AS (
  SELECT r.fish_code, r.genotype_rollup AS mom_genotype
  FROM public.v_fish_rich r
),
dad AS (
  SELECT r.fish_code, r.genotype_rollup AS dad_genotype
  FROM public.v_fish_rich r
)
SELECT
