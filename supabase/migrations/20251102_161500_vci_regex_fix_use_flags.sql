...
-- Recreate v_clutch_instances with the same shape as 160700, but
-- fix regexp_matches to use 'gi' flags instead of inline (?i).
...
-- Extract Tg tokens and inner codes with ordinals (use 'gi' flags)
geno_tokens AS (
  SELECT
    b.clutch_code,
    m.ord::int                                 AS ord,
...
-- Offspring genotype (full Tg tokens) ; separated with a space (NEW COLUMN)
geno_tokens_join AS (
  SELECT clutch_code,
...
SELECT
  ...
  -- Full Tg tokens (semicolon + space)
  gtj.clutch_genotype_tokens,

  -- Lineage (unchanged names, now using corrected codes)
  CASE
