...
-- 1) Drop dependents first so we can change column sets
...
-- 2) v_tanks: membership-only (open), active tanks only
...
-- 3) v_fish_rich: enriched; counts via v_tanks (no fallbacks)
...
-- 4) v_cross_clutch_instances: final version (genotypes via v_fish_genotypes or rollup)
