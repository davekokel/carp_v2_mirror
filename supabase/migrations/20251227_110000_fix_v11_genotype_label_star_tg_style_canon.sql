  ...
  SELECT
    ...
    CASE
      ...
      ELSE NULLIF(
        string_agg(
          tld.tg_label,
          '; ' ORDER BY tld.sort_kind, tld.sort_prefix, tld.sort_num, tld.base_code_norm, tld.tg_label
        ),
        ''
      )
    END AS tg_style_canon
  FROM g
  LEFT JOIN tg_labels_distinct tld
    ON tld.genotype_v11_id = g.genotype_v11_id
  GROUP BY g.genotype_v11_id
)
SELECT
  g.genotype_v11_id,
  g.genotype_code,
