...
SELECT
  ...
  roll.fluors_rollup,          -- already fixed to "true fusions" in 201600
  (
    SELECT string_agg(
  ...
  -- NEW: markers_rollup = full legacy fluor rollup (genotype + treatments + dyes)
  roll.markers_rollup
...
LEFT JOIN LATERAL (
  SELECT
    ...
    -- TRUE fusions only: genotype-side labels from zf_* fields
    (
    ...
    -- fluors_rollup = TRUE FUSIONS ONLY (same as genotype_fusions_rollup)
    (
      SELECT string_agg(DISTINCT t.t, '; ' ORDER BY t.t)
      FROM regexp_split_to_table(
             COALESCE(r.zf_female_genotype, '') || ',' || COALESCE(r.zf_male_genotype, ''),
    ...
    -- markers_rollup = full legacy fluor rollup: genotype + treatments + dyes
    (
      SELECT string_agg(DISTINCT t.t, '; ' ORDER BY t.t)
      FROM regexp_split_to_table(
