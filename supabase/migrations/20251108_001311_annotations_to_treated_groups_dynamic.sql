...
-- Copy clutch-level annotations to treated_clutch with schema-aware join.
DO $$
DECLARE
  has_ci_id   boolean;
  has_ci_code boolean;
...
BEGIN
  ...
  sql_text := 'WITH src AS ( ' ||
              '  SELECT tc.id AS treated_clutch_id, ja.annotation_id, ja.value_num, ja.value_text, ' ||
              '         COALESCE(ja.created_by, '''') AS created_by, ja.created_at ' ||
              ...
              '   AND j.target_id     = s.treated_clutch_id ' ||
              ...
              'SELECT ''treated_clutch'', treated_clutch_id, annotation_id, value_num, value_text, created_by, created_at ' ||
              'FROM dupe_filtered';
