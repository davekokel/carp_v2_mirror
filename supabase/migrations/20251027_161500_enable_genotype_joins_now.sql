-- Force-enable genotype joins now that v_fish_rich_derive_pretties exists
...
ci_agg AS (
  SELECT ci.clutch_plan_id, COUNT(ci.id)::int AS cross_count, MAX(ci.cross_date) AS last_cross_date
  FROM ci
  GROUP BY ci.clutch_plan_id
),
cl AS (
  SELECT pc.clutch_plan_id, cl2.clutch_instance_code, cl2.created_at, cl2.annotated_at
  FROM pc
  LEFT JOIN public.crosses x ON x.id = pc.planned_cross_id
  LEFT JOIN public.cross_instances ci2 ON ci2.cross_id = x.id
