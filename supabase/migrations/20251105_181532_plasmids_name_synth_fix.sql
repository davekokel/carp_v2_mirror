...
-- Pretty rollup for one plasmid (fluor::tag, comma-separated)
...
AS $$
  ...
  SELECT NULLIF(string_agg(lbl, ', ' ORDER BY lbl), '')
  FROM lbls;
$$;

-- Decide display name: use rollup if present; else nickname; else code
...
-- BEFORE INSERT: set NEW.name if empty (links may not exist yet)
...
-- AFTER change in link table: recompute name for affected plasmid
...
-- AFTER relevant fusion changes: refresh names for linked plasmids
