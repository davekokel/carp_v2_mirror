...
-- sequence for human-friendly fish codes like FSH-00000001
DO $$
BEGIN
...
DECLARE
  ...
  v_code text;
BEGIN
  -- normalize the identity key we were given
  v_key  := COALESCE(p_identity_key,'')::text;
  ...
  -- generate a new code like FSH-00000001
