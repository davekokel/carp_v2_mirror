...
begin
  if exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='crosses' and column_name='cross_name') then
    alter table public.crosses rename column cross_name to cross_name_code;
  end if;
  if exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='crosses' and column_name='cross_nickname') then
...
as $$
begin
  -- Always set code name from codes
