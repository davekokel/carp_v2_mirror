...
-- Always drop dependent views first (safe if missing)
...
-- Ensure ft_injection_mixes exists (non-destructive)
DO $$
BEGIN
...
-- Wire/ensure FK chain for mixes
...
-- Re/normalize lookup views to stable (code,label)
DO $$
...
-- Marker rollup view (proteins + dyes)
...
-- Fish-level view, resilient to column names (fixed typo: fish_code_col FROM)
DO $$
...
-- Validator view (stray join rows)
