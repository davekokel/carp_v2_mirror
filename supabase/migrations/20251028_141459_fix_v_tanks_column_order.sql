...
-- Drop dependents first so we can recreate v_tanks with the right column order.
DROP VIEW IF EXISTS public.v_fish_rich;

-- Recreate v_tanks with canonical column order and names
-- (tank_uuid, tank_code, status, created_at, fish_code, fish_uuid, started_at, ended_at, is_active)
...
-- Recreate v_fish_rich exactly as earlier (depends on v_tanks)
