BEGIN;
ALTER TABLE public.ft_proteins ADD COLUMN IF NOT EXISTS created_at timestamptz;
ALTER TABLE public.ft_dyes     ADD COLUMN IF NOT EXISTS created_at timestamptz;
UPDATE public.ft_proteins SET created_at=now() WHERE created_at IS NULL;
UPDATE public.ft_dyes     SET created_at=now() WHERE created_at IS NULL;
ALTER TABLE public.ft_proteins ALTER COLUMN created_at SET DEFAULT now();
ALTER TABLE public.ft_proteins ALTER COLUMN created_at SET NOT NULL;
ALTER TABLE public.ft_dyes     ALTER COLUMN created_at SET DEFAULT now();
ALTER TABLE public.ft_dyes     ALTER COLUMN created_at SET NOT NULL;
...
BEGIN
  IF to_regclass('public.fluors') IS NOT NULL THEN
    SELECT column_name INTO k FROM information_schema.columns
      WHERE table_schema='public' AND table_name='fluors' AND data_type IN ('text','character varying')
  ...
  ELSIF to_regclass('public.fluor_names') IS NOT NULL THEN
    SELECT column_name INTO k FROM information_schema.columns
      WHERE table_schema='public' AND table_name='fluor_names' AND data_type IN ('text','character varying')
        AND column_name IN ('code','fluor_code','name')
      ORDER BY CASE column_name WHEN 'code' THEN 1 WHEN 'fluor_code' THEN 2 WHEN 'name' THEN 3 ELSE 9 END LIMIT 1;
    SELECT CASE WHEN EXISTS (
  ...
  IF to_regclass('public.tags') IS NOT NULL THEN
    SELECT column_name INTO k FROM information_schema.columns
      WHERE table_schema='public' AND table_name='tags' AND data_type IN ('text','character varying')
  ...
  ELSIF to_regclass('public.tag_names') IS NOT NULL THEN
    SELECT column_name INTO k FROM information_schema.columns
      WHERE table_schema='public' AND table_name='tag_names' AND data_type IN ('text','character varying')
        AND column_name IN ('code','tag_code','name')
      ORDER BY CASE column_name WHEN 'code' THEN 1 WHEN 'tag_code' THEN 2 WHEN 'name' THEN 3 ELSE 9 END LIMIT 1;
    SELECT CASE WHEN EXISTS (
  ...
  IF to_regclass('public.dyes') IS NOT NULL THEN
    SELECT column_name INTO k FROM information_schema.columns
      WHERE table_schema='public' AND table_name='dyes' AND data_type IN ('text','character varying')
