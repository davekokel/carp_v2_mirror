...
WITH
-- 1) Genotype "pretty" label per fish (from genotypes)
genotype_labels AS (
  SELECT
    jfg.fish_id,
    string_agg(
...
-- 2) Genotype base codes per fish (from join_fish_transgene_alleles)
...
-- 3) Genotype fluors per fish (via plasmids / RNAs → fusions → fluors)
...
-- 4) Treatment base codes per fish (dyes + plasmids + RNAs)
...
-- 5) Treatment fluors per fish (treatment plasmids/RNAs → fusions → fluors)
...
-- 6) All base codes per fish (genotype + treatment)
all_base_codes AS (
  SELECT
    fish_id,
    string_agg(
...
-- 7) All fluors per fish (genotype + treatment)
all_fluor_codes AS (
  SELECT
    fish_id,
    fluor_code
