...
-- Ensure pgcrypto (for gen_random_uuid / digest)
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Drop existing upsert with the exact signature so we can change OUT rowtype
...
-- Recreate guarded upsert (skips writes to identity_hash if it's a generated column)
