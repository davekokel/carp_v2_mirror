...
treats AS (
    SELECT
        c.clutch_code,
        string_agg(DISTINCT t1.treat_code, '||' ORDER BY t1.treat_code) AS treat_codes,
    ...
    LEFT JOIN public.treatments t1 ON t1.id = jct.treatment_id
    LEFT JOIN public.treatment_mixes tm ON tm.treatment_id = t1.id
