...
CREATE TYPE IF NOT EXISTS public.tank_status AS ENUM ('vacant','occupied','quarantine','maintenance','retired','decommissioned');
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema='public' AND table_name='fish_tank_assignments'
  ) THEN
    IF EXISTS (
      SELECT 1 FROM pg_trigger tg
      JOIN pg_class c ON c.oid=tg.tgrelid
      JOIN pg_namespace n ON n.oid=c.relnamespace
      WHERE n.nspname='public' AND c.relname='fish_tank_assignments' AND tg.tgname='trg_fta_sync_status_iud'
    ) THEN
      EXECUTE 'DROP TRIGGER trg_fta_sync_status_iud ON public.fish_tank_assignments';
    END IF;
  END IF;
END $$;
