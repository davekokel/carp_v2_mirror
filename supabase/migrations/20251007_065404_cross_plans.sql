...
CREATE TABLE IF NOT EXISTS public.cross_plans (
  ...
  tank_a_id     uuid NOT NULL,
  tank_b_id     uuid NOT NULL,
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema='public' AND table_name='transgene_alleles' AND column_name='transgene_base_code'
  ) AND EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='transgene_alleles' AND column_name='allele_number'
  ) THEN
