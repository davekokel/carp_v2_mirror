...
SELECT
  ...
  -- compatibility fields expected by older queries/pages
  CASE WHEN b.date_birth IS NOT NULL THEN (CURRENT_DATE - b.date_birth) ELSE NULL::int END AS age_days,
  CASE WHEN b.date_birth IS NOT NULL THEN ((CURRENT_DATE - b.date_birth)/7)::int ELSE NULL::int END AS age_weeks,
  COALESCE(b.created_by, '') AS created_by_enriched,
