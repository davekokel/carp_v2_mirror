...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'chk_auto_fish_code_format' AND conrelid = 'public.fish'::regclass
  ) THEN
    ALTER TABLE public.fish
      ADD CONSTRAINT chk_auto_fish_code_format
      CHECK (
        auto_fish_code IS NULL OR auto_fish_code ~ '^FSH-\d{4}-\d{3}$'
      );
  END IF;
END$$;
...
GRANT SELECT ON public.fish, public.transgenes, public.transgene_allele_catalog,
               public.fish_transgene_alleles, public.treatments, public.fish_treatments,
               public.tank_assignments
