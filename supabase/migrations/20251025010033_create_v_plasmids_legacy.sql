...
-- Recreate the legacy view name used by Streamlit pages
...
select
  ...
  -- RNA fields not in baseline; expose as NULLs for compat
...
-- Helper index to speed common ORDER BY v_code queries (if missing)
do $$
begin
  if not exists (
    select 1 from pg_indexes
    where schemaname='public' and indexname='ix_plasmids_v_code'
  ) then
    create index ix_plasmids_v_code on public.plasmids(code);
  end if;
end $$;
