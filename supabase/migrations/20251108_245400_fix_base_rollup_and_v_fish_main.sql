...
-- 1) Recreate the per-base rollup WITH ft_code
...
SELECT fish_code, ft_code, fluors, tags
FROM agg;

-- 2) Recreate v_fish_main to include base_* and match page columns
...
SELECT
  ...
  r.tags,                        -- fish-level
  ''::text AS dyes,              -- placeholder
  rb.fluors AS base_fluors,      -- base-scoped (this base only)
  rb.tags   AS base_tags
FROM public.fish f
