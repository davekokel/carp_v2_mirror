-- Fix v_tank_pairs: join clutches via cross_instances (not cl.tank_pair_code)
DO $$
DECLARE
  mcol text;
  dcol text;
...
BEGIN
  ...
  ELSE RAISE EXCEPTION 'tank_pairs missing mother/father tank id columns'; END IF;

  -- pick fish view and genotype column
  ...
  IF fish_view IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=split_part(fish_view,'.',2) AND column_name='transgene_pretty_name') THEN
    gene_col:='transgene_pretty_name';
  ELSIF fish_view IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=split_part(fish_view,'.',2) AND column_name='genotype_rollup') THEN
    gene_col:='genotype_rollup';
  ELSE
    gene_col:=NULL;
  END IF;

  -- fish_pair_code present?
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='tank_pairs' AND column_name='fish_pair_code'
  ) INTO has_fp;

  -- drop old, rebuild correct view
  ...
  $fmt$,
    ...
    CASE WHEN fish_view IS NULL OR gene_col IS NULL THEN 'NULL::text' ELSE format('COALESCE(vfm.%I, '''')', gene_col) END,
    CASE WHEN fish_view IS NULL OR gene_col IS NULL THEN 'NULL::text' ELSE format('COALESCE(vfd.%I, '''')', gene_col) END,
    mcol, dcol,
