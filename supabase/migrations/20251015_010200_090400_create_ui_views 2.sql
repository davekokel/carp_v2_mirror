...
create or replace view public.v_containers_crossing_candidates as  SELECT id,
    container_type,
    label,
    status,
    created_by,
...
create or replace view public.v_containers_live as  SELECT id,
    container_type,
    label,
    status,
    created_by,
...
create or replace view public.v_fish_standard_clean as  WITH vs AS (
 ...
 SELECT fish_code,
    ...
    created_by
   FROM roll;;
create or replace view public.v_label_jobs_recent as  SELECT id,
    entity_type,
    entity_id,
    template,
    media,
...
create or replace view public.v_tank_pairs_overview as  SELECT tp.id,
    tp.tank_pair_code,
    tp.concept_id,
...
create or replace view public.vw_bruker_mounts_enriched as  SELECT mount_code,
...
create or replace view public.vw_clutches_concept_overview as  WITH base AS (
         SELECT cp.id AS clutch_plan_id,
...
create or replace view public.vw_clutches_overview_human as  WITH base AS (
...
create or replace view public.vw_cross_runs_overview as  WITH cl AS (
...
create or replace view public.vw_crosses_concept as  WITH runs AS (
...
create or replace view public.vw_fish_overview_with_label as  WITH base AS (
         SELECT f.fish_code,
            f.name,
            f.nickname,
...
create or replace view public.vw_fish_standard as  WITH base AS (
         SELECT f.id,
            f.fish_code,
...
create or replace view public.vw_label_rows as  WITH base AS (
         SELECT f.id,
            f.fish_code,
            f.name,
            f.nickname,
 ...
 SELECT b.id,
   ...
   FROM base b
     ...
     LEFT JOIN batch USING (fish_code)
  ORDER BY b.fish_code;;
create or replace view public.vw_planned_clutches_overview as  WITH x AS (
         SELECT cp.id AS clutch_plan_id,
            pc.id AS planned_cross_id,
            cp.clutch_code,
            cp.planned_name AS clutch_name,
...
create or replace view public.vw_plasmids_overview as  SELECT p.id,
    p.code,
    p.name,
    p.nickname,
    p.fluors,
