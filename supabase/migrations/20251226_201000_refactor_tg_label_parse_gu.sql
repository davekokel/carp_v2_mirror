...
SELECT
  ...
  NULLIF(btrim(max(gl.tg_style_canon)), '') AS tx_gt_tg,
  NULLIF(btrim(max(gl.fluor_tag_style)), '') AS tx_gt_fluortag,
  NULLIF(btrim(max(gl.fluor_organelle_style)), '') AS tx_gt_fluororganelle,
  ch.n_channels_total,
  ch.n_channels_kept,
