...
-- Drop the clutch views so we can change column names/order
DROP VIEW IF EXISTS public.v_clutch_instances;
DROP VIEW IF EXISTS public.v_clutch_instances_base;

-- Ensure the resolver exists (normalized; no legacy fallbacks)
...
-- Rebuild the base clutch view with the tight column set
...
-- Outer passthrough (keeps the familiar name)
