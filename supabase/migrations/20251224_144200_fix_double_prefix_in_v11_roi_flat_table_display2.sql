...
SELECT
  ...
  CASE
    WHEN coalesce(btrim(r.tx_gt_tg),'') <> '' AND r.tx_gt_tg LIKE '% > %'
      THEN r.tx_gt_tg
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_tg),'') <> ''
      THEN tt.treat_basecodes_typed || ' > ' || r.tx_gt_tg
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_tg),'') = ''
      THEN tt.treat_basecodes_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
  ...
  CASE
    ...
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluortag),'') <> ''
      THEN tt.treat_basecodes_typed || ' > ' || r.tx_gt_fluortag
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluortag),'') = ''
      THEN tt.treat_basecodes_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
  ...
  CASE
    ...
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluororganelle),'') <> ''
      THEN tt.treat_basecodes_typed || ' > ' || r.tx_gt_fluororganelle
    WHEN coalesce(btrim(tt.treat_basecodes_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluororganelle),'') = ''
      THEN tt.treat_basecodes_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
