-- Recreate public.v_tank_pairs from current schema (tank_pairs + v_tanks + cross_instances + optional v_fish_*).
DO $$
DECLARE
  mcol text;
  dcol text;
  ...
  dyn_sql text;
BEGIN
  -- Mother/Father columns on tank_pairs
  ...
  -- Pick fish view (+ genotype col) if available
  ...
  -- Optional fish_pair_code on tank_pairs
  SELECT EXISTS (
