...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='fish' AND policyname='app_rw_select_fish'
  ) THEN
    CREATE POLICY app_rw_select_fish ON public.fish FOR SELECT TO app_rw USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='fish' AND policyname='app_rw_insert_fish'
  ) THEN
    CREATE POLICY app_rw_insert_fish ON public.fish FOR INSERT TO app_rw WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='fish' AND policyname='app_rw_update_fish'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='transgene_alleles' AND policyname='app_rw_select_tga'
  ) THEN
    CREATE POLICY app_rw_select_tga ON public.transgene_alleles FOR SELECT TO app_rw USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='transgene_alleles' AND policyname='app_rw_insert_tga'
  ) THEN
    CREATE POLICY app_rw_insert_tga ON public.transgene_alleles FOR INSERT TO app_rw WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='transgene_alleles' AND policyname='app_rw_update_tga'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='fish_transgene_alleles' AND policyname='app_rw_select_fta'
  ) THEN
    CREATE POLICY app_rw_select_fta ON public.fish_transgene_alleles FOR SELECT TO app_rw USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='fish_transgene_alleles' AND policyname='app_rw_insert_fta'
  ) THEN
    CREATE POLICY app_rw_insert_fta ON public.fish_transgene_alleles FOR INSERT TO app_rw WITH CHECK (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='fish_transgene_alleles' AND policyname='app_rw_update_fta'
  ) THEN
