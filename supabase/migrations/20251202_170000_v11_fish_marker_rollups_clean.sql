...
DROP VIEW IF EXISTS public.v11_fish_marker_rollups CASCADE;
...
expanded AS (
  SELECT
    b.fish_instance_id,
    split_part(trim(both from code.code), ':' , 1) AS construct_base_code
  FROM base b,
  LATERAL regexp_split_to_table(
    COALESCE(b.genotype_basecodes, ''),
    '\\s*\\|\\|\\s*'
  ) AS code(code)
  WHERE trim(both from code.code) <> ''
),
joined AS (
  SELECT
    e.fish_instance_id,
...
'Per-fish-instance fluor/tags and organelle-fluor rollups derived directly from constructs + v_constructs_overview (hyphenated fluor-tag and localization-fluor).';
