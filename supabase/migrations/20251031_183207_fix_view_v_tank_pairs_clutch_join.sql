-- Fix v_tank_pairs: correct clutch join via cross_instances; robust to column/view variants.
DO $$
DECLARE
  mcol text;
  dcol text;
  ...
  dyn_sql text;
BEGIN
  -- detect mother/father tank id columns
  ...
  ELSE
    RAISE EXCEPTION 'public.tank_pairs missing mother/father tank id columns';
  END IF;

  -- pick fish view and genotype column if present
  ...
  IF fish_view IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.columns
                                       WHERE table_schema='public'
                                         AND table_name=split_part(fish_view,'.',2)
                                         AND column_name='transgene_pretty_name') THEN
    gene_col:='transgene_pretty_name';
  ELSIF fish_view IS NOT NULL AND EXISTS (SELECT 1 FROM information_schema.columns
                                       WHERE table_schema='public'
                                         AND table_name=split_part(fish_view,'.',2)
