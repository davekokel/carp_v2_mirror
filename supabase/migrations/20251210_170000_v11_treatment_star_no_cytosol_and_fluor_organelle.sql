...
-- Rebuild v11_treatment_star to:
--  1) Stop inventing fluor::cytosol / cytosol-fluor when tag/localization is missing.
--  2) Use fluor-organelle style instead of organelle-fluor for the org-style label.
...
fusion_bits AS (
  SELECT
    ...
    f.tag_pos,
    -- fluor-tag style:
    --   with tag  -> fluor::tag(tag_pos)
    --   no tag    -> fluor  (do NOT invent ::cytosol)
    CASE
      WHEN tg.id IS NULL
    ...
    -- org style:
    --   with localization -> fluor-organelle
    --   no localization   -> fluor  (do NOT invent cytosol-*)
    CASE
