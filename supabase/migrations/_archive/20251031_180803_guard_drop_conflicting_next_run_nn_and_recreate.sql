...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conrelid='public.cross_instances'::regclass AND conname='uq_tankpair_run_nn'
  ) AND NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conrelid='public.cross_instances'::regclass AND conname='uq_tank_pairs_run_nn'
  ) THEN
...
BEGIN
  ...
  SELECT COALESCE(MAX(run_nn),0)+1 INTO v FROM public.cross_instances WHERE tank_pair_code=p_tank_pair_code;
  RETURN v;
END
$$;
