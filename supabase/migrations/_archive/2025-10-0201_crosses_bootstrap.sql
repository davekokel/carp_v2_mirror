...
BEGIN
  ...
  IF to_regclass('public.fish') IS NOT NULL THEN
    ...
    -- fish.cross_id pointer to crosses
    PERFORM 1 FROM information_schema.columns
      WHERE table_schema='public' AND table_name='fish' AND column_name='cross_id';
    IF NOT FOUND THEN
      ALTER TABLE public.fish ADD COLUMN cross_id uuid;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fish_cross_fk') THEN
