-- Golden clutch instances view: canonical fields, no suffixes.
-- Uses existing v_clutch_treatments; coalesces genotype.
CREATE OR REPLACE VIEW public.v_clutch_instances AS
WITH src AS (
  SELECT
    v.*,                             -- existing columns from your effective base
...
SELECT
  ...
  -- rollups (canonical names, no _resolved)
