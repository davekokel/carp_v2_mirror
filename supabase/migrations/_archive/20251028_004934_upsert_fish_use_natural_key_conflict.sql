...
DECLARE
  v_code text;
BEGIN
  -- Authoritative code: mint on insert only
  ...
  VALUES (
    v_code,
    COALESCE(trim(p_seed_batch_id), ''),  -- columns are NOT NULL
    COALESCE(trim(p_name), ''),           -- NOT NULL
    NULLIF(trim(p_nick), ''),
    NULLIF(trim(p_bg), ''),
    NULLIF(trim(p_stage), ''),
    p_dob,                                -- consider enforcing NOT NULL upstream
    NULLIF(trim(p_desc), ''),
    NULLIF(trim(p_notes), ''),
    NULLIF(trim(p_by), '')
  )
