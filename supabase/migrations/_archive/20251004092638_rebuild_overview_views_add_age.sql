...
select
  ...
  case when p.date_birth is not null then (current_date - p.date_birth) else null end as age_days,
  case when p.date_birth is not null then floor((current_date - p.date_birth)::numeric / 7)::int else null end as age_weeks,
