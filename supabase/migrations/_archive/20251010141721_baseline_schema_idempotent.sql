...
-- removed transaction_timeout (not supported on vanilla PG)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE t.typname='container_status' AND n.nspname='public') THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace
    WHERE t.typname='cross_plan_status' AND n.nspname='public') THEN
...
DO LANGUAGE plpgsql $$
  ...
  IF NOT EXISTS (
    ...
    WHERE c.relkind = 'S'
      AND c.relname = 'clutch_instance_code_seq'
...
SELECT public.safe_drop_view('public','seed_batches');
SELECT public.safe_drop_view('public','seed_batches');
CREATE VIEW public.seed_batches AS SELECT NULL::text AS seed_batch_id,
...
SELECT public.safe_drop_view('public','v_containers_crossing_candidates');
SELECT public.safe_drop_view('public','v_containers_crossing_candidates');
CREATE VIEW public.v_containers_crossing_candidates AS SELECT id_uuid,
    container_type,
    label,
    status,
    created_by,
...
SELECT public.safe_drop_view('public','v_containers_live');
SELECT public.safe_drop_view('public','v_containers_live');
CREATE VIEW public.v_containers_live AS SELECT id_uuid,
    container_type,
    label,
    status,
    created_by,
...
SELECT public.safe_drop_view('public','v_cross_plan_runs_enriched');
SELECT public.safe_drop_view('public','v_cross_plan_runs_enriched');
CREATE VIEW public.v_cross_plan_runs_enriched AS SELECT r.id,
    r.plan_id,
    r.seq,
    r.planned_date,
    r.status,
...
SELECT public.safe_drop_view('public','v_cross_plans_enriched');
SELECT public.safe_drop_view('public','v_cross_plans_enriched');
CREATE VIEW public.v_cross_plans_enriched AS SELECT p.id,
    p.plan_date,
    p.status,
    p.created_by,
    p.note,
...
SELECT public.safe_drop_view('public','v_crosses_status');
SELECT public.safe_drop_view('public','v_crosses_status');
CREATE VIEW public.v_crosses_status AS SELECT id_uuid,
    mother_code,
    father_code,
    planned_for,
    created_by,
...
SELECT public.safe_drop_view('public','v_fish_label_fields');
SELECT public.safe_drop_view('public','v_fish_label_fields');
CREATE VIEW public.v_fish_label_fields AS SELECT fish_code,
    nickname,
    name,
...
SELECT public.safe_drop_view('public','v_fish_living_tank_counts');
SELECT public.safe_drop_view('public','v_fish_living_tank_counts');
CREATE VIEW public.v_fish_living_tank_counts AS SELECT m.fish_id,
...
SELECT public.safe_drop_view('public','v_fish_overview');
SELECT public.safe_drop_view('public','v_fish_overview');
CREATE VIEW public.v_fish_overview AS SELECT fish_code,
    name,
    nickname,
    line_building_stage,
    date_birth,
...
SELECT public.safe_drop_view('public','v_fish_overview_canonical');
SELECT public.safe_drop_view('public','v_fish_overview_canonical');
CREATE VIEW public.v_fish_overview_canonical AS SELECT fish_code,
    name,
    nickname,
    line_building_stage,
    date_birth,
...
SELECT public.safe_drop_view('public','v_label_jobs_recent');
SELECT public.safe_drop_view('public','v_label_jobs_recent');
CREATE VIEW public.v_label_jobs_recent AS SELECT id_uuid,
    entity_type,
    entity_id,
    template,
    media,
...
SELECT public.safe_drop_view('public','vw_clutches_concept_overview');
SELECT public.safe_drop_view('public','vw_clutches_concept_overview');
CREATE VIEW public.vw_clutches_concept_overview AS WITH base AS (
...
SELECT public.safe_drop_view('public','vw_clutches_overview_human');
SELECT public.safe_drop_view('public','vw_clutches_overview_human');
CREATE VIEW public.vw_clutches_overview_human AS WITH base AS (
...
SELECT public.safe_drop_view('public','vw_cross_runs_overview');
SELECT public.safe_drop_view('public','vw_cross_runs_overview');
CREATE VIEW public.vw_cross_runs_overview AS WITH cl AS (
...
SELECT public.safe_drop_view('public','vw_crosses_concept');
SELECT public.safe_drop_view('public','vw_crosses_concept');
CREATE VIEW public.vw_crosses_concept AS WITH runs AS (
...
SELECT public.safe_drop_view('public','vw_fish_overview_with_label');
SELECT public.safe_drop_view('public','vw_fish_overview_with_label');
CREATE VIEW public.vw_fish_overview_with_label AS WITH base AS (
         SELECT f.fish_code,
            f.name,
            f.nickname,
...
SELECT public.safe_drop_view('public','vw_fish_standard');
SELECT public.safe_drop_view('public','vw_fish_standard');
CREATE VIEW public.vw_fish_standard AS WITH base AS (
...
SELECT public.safe_drop_view('public','vw_label_rows');
SELECT public.safe_drop_view('public','vw_label_rows');
CREATE VIEW public.vw_label_rows AS WITH base AS (
         SELECT f.id_uuid,
            f.fish_code,
            f.name,
            f.nickname,
...
SELECT public.safe_drop_view('public','vw_planned_clutches_overview');
SELECT public.safe_drop_view('public','vw_planned_clutches_overview');
CREATE VIEW public.vw_planned_clutches_overview AS WITH x AS (
         SELECT cp.id_uuid AS clutch_plan_id,
            pc.id_uuid AS planned_cross_id,
            cp.clutch_code,
            cp.planned_name AS clutch_name,
...
SELECT public.safe_drop_view('public','vw_plasmids_overview');
SELECT public.safe_drop_view('public','vw_plasmids_overview');
CREATE VIEW public.vw_plasmids_overview AS SELECT p.id_uuid,
    p.code,
    p.name,
    p.nickname,
    p.fluors,
...
DO LANGUAGE plpgsql $$
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'allele_nicknames_pkey'
      AND conrelid = 'public.allele_nicknames'::regclass
  ) THEN
...
DO LANGUAGE plpgsql $$
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clutch_containers_pkey'
      AND conrelid = 'public.clutch_containers'::regclass
  ) THEN
DO LANGUAGE plpgsql $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutch_containers_pkey' AND conrelid='public.clutch_containers'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutch_genotype_options_pkey' AND conrelid='public.clutch_genotype_options'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutch_plan_treatments_pkey' AND conrelid='public.clutch_plan_treatments'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutch_plans_pkey' AND conrelid='public.clutch_plans'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutch_treatments_pkey' AND conrelid='public.clutch_treatments'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='clutches_pkey' AND conrelid='public.clutches'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='container_status_history_pkey' AND conrelid='public.container_status_history'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.container_status_history ADD CONSTRAINT container_status_history_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='containers_pkey' AND conrelid='public.containers'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='cross_instances_pkey' AND conrelid='public.cross_instances'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='cross_plan_genotype_alleles_pkey' AND conrelid='public.cross_plan_genotype_alleles'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='cross_plan_runs_pkey' AND conrelid='public.cross_plan_runs'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='cross_plan_treatments_pkey' AND conrelid='public.cross_plan_treatments'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='cross_plans_pkey' AND conrelid='public.cross_plans'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='crosses_pkey' AND conrelid='public.crosses'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_code_audit_pkey' AND conrelid='public.fish_code_audit'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.fish_code_audit ADD CONSTRAINT fish_code_audit_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_pkey' AND conrelid='public.fish'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.fish ADD CONSTRAINT fish_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_seed_batches_pkey' AND conrelid='public.fish_seed_batches'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.fish_seed_batches ADD CONSTRAINT fish_seed_batches_pkey PRIMARY KEY (fish_id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_tank_memberships_pkey' AND conrelid='public.fish_tank_memberships'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_transgene_alleles_pkey' AND conrelid='public.fish_transgene_alleles'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.fish_transgene_alleles ADD CONSTRAINT fish_transgene_alleles_pkey PRIMARY KEY (fish_id, transgene_base_code, allele_number);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='fish_year_counters_pkey' AND conrelid='public.fish_year_counters'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.fish_year_counters ADD CONSTRAINT fish_year_counters_pkey PRIMARY KEY (year);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='injected_plasmid_treatments_pkey' AND conrelid='public.injected_plasmid_treatments'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.injected_plasmid_treatments ADD CONSTRAINT injected_plasmid_treatments_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='injected_rna_treatments_pkey' AND conrelid='public.injected_rna_treatments'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.injected_rna_treatments ADD CONSTRAINT injected_rna_treatments_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='label_items_pkey' AND conrelid='public.label_items'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.label_items ADD CONSTRAINT label_items_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='label_jobs_pkey' AND conrelid='public.label_jobs'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.label_jobs ADD CONSTRAINT label_jobs_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='load_log_fish_pkey' AND conrelid='public.load_log_fish'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.load_log_fish ADD CONSTRAINT load_log_fish_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='planned_crosses_pkey' AND conrelid='public.planned_crosses'::regclass AND contype='p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='plasmid_registry_pkey' AND conrelid='public.plasmid_registry'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.plasmid_registry ADD CONSTRAINT plasmid_registry_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='plasmids_pkey' AND conrelid='public.plasmids'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.plasmids ADD CONSTRAINT plasmids_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='rna_registry_pkey' AND conrelid='public.rna_registry'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.rna_registry ADD CONSTRAINT rna_registry_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='rnas_pkey' AND conrelid='public.rnas'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.rnas ADD CONSTRAINT rnas_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='selection_labels_pkey' AND conrelid='public.selection_labels'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.selection_labels ADD CONSTRAINT selection_labels_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='tank_requests_pkey' AND conrelid='public.tank_requests'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.tank_requests ADD CONSTRAINT tank_requests_pkey PRIMARY KEY (id_uuid);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='tank_year_counters_pkey' AND conrelid='public.tank_year_counters'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.tank_year_counters ADD CONSTRAINT tank_year_counters_pkey PRIMARY KEY (year);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='transgene_allele_counters_pkey' AND conrelid='public.transgene_allele_counters'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.transgene_allele_counters ADD CONSTRAINT transgene_allele_counters_pkey PRIMARY KEY (transgene_base_code);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='transgene_allele_registry_pkey' AND conrelid='public.transgene_allele_registry'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.transgene_allele_registry ADD CONSTRAINT transgene_allele_registry_pkey PRIMARY KEY (id);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='transgene_alleles_pkey' AND conrelid='public.transgene_alleles'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.transgene_alleles ADD CONSTRAINT transgene_alleles_pkey PRIMARY KEY (transgene_base_code, allele_number);
  END IF;
END $$;
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='transgenes_pkey' AND conrelid='public.transgenes'::regclass AND contype='p'
  ) THEN
    ALTER TABLE ONLY public.transgenes ADD CONSTRAINT transgenes_pkey PRIMARY KEY (transgene_base_code);
  END IF;
END $$;
--
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cc_clutch ON public.clutch_containers USING btree (clutch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cc_created_desc ON public.clutch_containers USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cc_selection ON public.clutch_containers USING btree (selection_label);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cgo_clutch ON public.clutch_genotype_options USING btree (clutch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clutches_batch ON public.clutches USING btree (batch_label);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clutches_created_desc ON public.clutches USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clutches_cross_id ON public.clutches USING btree (cross_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clutches_run_id ON public.clutches USING btree (run_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clutches_seed ON public.clutches USING btree (seed_batch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_containers_created_desc ON public.containers USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_containers_type_status ON public.containers USING btree (container_type, status);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cpga_plan ON public.cross_plan_genotype_alleles USING btree (plan_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cpt_plan ON public.cross_plan_treatments USING btree (plan_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cpt_plasmid ON public.cross_plan_treatments USING btree (plasmid_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cpt_rna ON public.cross_plan_treatments USING btree (rna_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plan_runs_date ON public.cross_plan_runs USING btree (planned_date);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plan_runs_plan ON public.cross_plan_runs USING btree (plan_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_created_by ON public.cross_plans USING btree (created_by);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_day_father ON public.cross_plans USING btree (plan_date, father_fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_day_mother ON public.cross_plans USING btree (plan_date, mother_fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_father ON public.cross_plans USING btree (father_fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_mother ON public.cross_plans USING btree (mother_fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_nick ON public.cross_plans USING btree (plan_nickname);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_plan_date ON public.cross_plans USING btree (plan_date);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_tank_a ON public.cross_plans USING btree (tank_a_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_tank_b ON public.cross_plans USING btree (tank_b_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_cross_plans_title ON public.cross_plans USING btree (plan_title);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_crosses_created_desc ON public.crosses USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_crosses_parents_code ON public.crosses USING btree (mother_code, father_code);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_csh_changed_at ON public.container_status_history USING btree (changed_at);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_csh_container ON public.container_status_history USING btree (container_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_ct_clutch ON public.clutch_treatments USING btree (clutch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_fsbm_fish_id ON public.fish_seed_batches_map USING btree (fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_fsbm_logged_at ON public.fish_seed_batches_map USING btree (logged_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_fsbm_seed_batch_id ON public.fish_seed_batches_map USING btree (seed_batch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_ftm_container ON public.fish_tank_memberships USING btree (container_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_ftm_fish ON public.fish_tank_memberships USING btree (fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_ftm_fish_id ON public.fish_tank_memberships USING btree (fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_items_job_seq ON public.label_items USING btree (job_id, seq);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_items_request ON public.label_items USING btree (request_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_items_tank ON public.label_items USING btree (tank_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_jobs_entity ON public.label_jobs USING btree (entity_type, entity_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_jobs_requested_by ON public.label_jobs USING btree (requested_by, requested_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_label_jobs_status ON public.label_jobs USING btree (status, requested_at DESC);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_load_log_fish_fish_id ON public.load_log_fish USING btree (fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_planned_crosses_clutch ON public.planned_crosses USING btree (clutch_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_plasmid_registry_code ON public.plasmid_registry USING btree (plasmid_code);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_rna_registry_code ON public.rna_registry USING btree (rna_code);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_rnas_source_plasmid ON public.rnas USING btree (source_plasmid_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_ta_base_nick_ci ON public.transgene_alleles USING btree (transgene_base_code, lower(allele_nickname));
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_vfltc_fish ON public.fish_tank_memberships USING btree (fish_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS ix_injected_rna_treatments_rna ON public.injected_rna_treatments USING btree (rna_id);
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS ix_registry_base_code ON public.transgene_allele_registry USING btree (base_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uniq_registry_base_legacy ON public.transgene_allele_registry USING btree (base_code, legacy_label) WHERE ((base_code IS NOT NULL) AND (legacy_label IS NOT NULL));
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uniq_registry_modern_key ON public.transgene_allele_registry USING btree (transgene_base_code, allele_nickname) WHERE ((transgene_base_code IS NOT NULL) AND (allele_nickname IS NOT NULL));
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_clutch_plans_clutch_code ON public.clutch_plans USING btree (clutch_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_clutches_instance_code ON public.clutches USING btree (clutch_instance_code) WHERE (clutch_instance_code IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_clutches_planned_by_date ON public.clutches USING btree (planned_cross_id, date_birth);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_containers_tank_code ON public.containers USING btree (tank_code) WHERE (tank_code IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_crosses_concept_pair ON public.crosses USING btree (upper(TRIM(BOTH FROM mother_code)), upper(TRIM(BOTH FROM father_code)));
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_crosses_cross_code ON public.crosses USING btree (cross_code) WHERE (cross_code IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_fsbm_batch_fish ON public.fish_seed_batches_map USING btree (seed_batch_id, fish_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_fta_fish_base ON public.fish_transgene_alleles USING btree (fish_id, transgene_base_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_ftm_fish_open ON public.fish_tank_memberships USING btree (fish_id) WHERE (left_at IS NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_ipt_natural ON public.injected_plasmid_treatments USING btree (fish_id, plasmid_id, at_time, amount, units, note);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_irt_natural ON public.injected_rna_treatments USING btree (fish_id, rna_id, at_time, amount, units, note);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_label_jobs_dedupe ON public.label_jobs USING btree (dedupe_hash) WHERE (dedupe_hash IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_planned_crosses_clutch_parents_canonical ON public.planned_crosses USING btree (clutch_id, mother_tank_id, father_tank_id) WHERE ((is_canonical = true) AND (mother_tank_id IS NOT NULL) AND (father_tank_id IS NOT NULL));
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_planned_crosses_cross_code ON public.planned_crosses USING btree (cross_code) WHERE (cross_code IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_plasmids_code ON public.plasmids USING btree (code);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_rna_txn_dedupe ON public.injected_rna_treatments USING btree (fish_id, rna_id, COALESCE(at_time, '1969-12-31 16:00:00-08'::timestamp with time zone), COALESCE(amount, (0)::numeric), COALESCE(units, ''::text), COALESCE(note, ''::text));
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_rnas_one_per_plasmid ON public.rnas USING btree (source_plasmid_id) WHERE (source_plasmid_id IS NOT NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS IF NOT EXISTS uq_tar_base_number ON public.transgene_allele_registry USING btree (transgene_base_code, allele_number);
