...
dups AS (
    SELECT
        ...
        n.clutch_id
    FROM norm AS n
    INNER JOIN canon AS c
        ON
            n.clutch_id = c.clutch_id
...
WITH moved AS (
    ...
    FROM pc_dups_map AS d
    WHERE
        c.planned_cross_id = d.dup_id
        AND NOT EXISTS (
            SELECT 1
            FROM public.clutches AS c2
            WHERE
                c2.planned_cross_id = d.canonical_id
                AND (
                    (c.date_birth IS NULL AND c2.date_birth IS NULL)
                    OR (c.date_birth IS NOT NULL AND c2.date_birth IS NOT NULL AND c2.date_birth = c.date_birth)
                )
        )
    RETURNING c.id_uuid
)
...
WHERE
    pc.id_uuid = d.dup_id
    AND NOT EXISTS (
        SELECT 1 FROM public.clutches AS c
        WHERE c.planned_cross_id = pc.id_uuid
    );
