-- Tanks state machine + history + assignments (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace
                 WHERE n.nspname='public' AND t.typname='tank_status') THEN
    CREATE TYPE public.tank_status AS ENUM (
      'vacant', 'occupied', 'quarantine', 'maintenance', 'retired', 'decommissioned'
    );
  END IF;
END $$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables
                 WHERE table_schema='public' AND table_name='tanks') THEN
  ...
  END IF;
END $$;
CREATE TABLE IF NOT EXISTS public.tank_status_history (
...
CREATE INDEX IF NOT EXISTS ix_tsh_tank_id_changed_at ON public.tank_status_history(tank_id, changed_at DESC);
...
LEFT JOIN public.tank_status_history h
  ON h.tank_id = l.tank_id AND h.changed_at = l.changed_at;
CREATE TABLE IF NOT EXISTS public.fish_tank_assignments (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_fta_fish_one_open
  ON public.fish_tank_assignments(fish_id)
...
RETURNS void LANGUAGE sql AS $$
  INSERT INTO public.tank_status_history (tank_id, status, reason)
  VALUES (p_tank_id, p_status, p_reason)
$$;
...
BEGIN
  ...
  SELECT coalesce(n_fish_open, 0) INTO v_open
  FROM public.v_tank_occupancy WHERE tank_id = v_tank_id;
  SELECT status INTO v_cur_status
  FROM public.v_tanks_current_status WHERE tank_id = v_tank_id;
...
CREATE OR REPLACE FUNCTION public.ensure_tank(p_tank_code text,
                                              p_rack text DEFAULT NULL,
                                              p_position text DEFAULT NULL,
                                              p_status public.tank_status DEFAULT NULL,
...
BEGIN
  ...
  IF v_exists IS NULL THEN
    INSERT INTO public.tanks(tank_code, rack, position) VALUES (p_tank_code, p_rack, p_position)
    RETURNING tank_id INTO v_tank_id;
  ...
  RETURN v_tank_id;
END $$;
CREATE OR REPLACE FUNCTION public.move_fish_to_tank(p_fish_id bigint,
                                                    p_tank_code text,
                                                    p_rack text DEFAULT NULL,
                                                    p_position text DEFAULT NULL,
...
BEGIN
  ...
  UPDATE public.fish_tank_assignments
  SET end_at = now()
  WHERE fish_id = p_fish_id AND end_at IS NULL;
  INSERT INTO public.fish_tank_assignments(fish_id, tank_id, note)
  VALUES (p_fish_id, v_tank_id, p_note);
END $$;
