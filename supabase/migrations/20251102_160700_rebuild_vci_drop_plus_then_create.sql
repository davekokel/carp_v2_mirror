...
-- 0) Drop the dependent view so we can change the signature of v_clutch_instances
DROP VIEW IF EXISTS public.v_clutch_instances_plus;

-- 1) Drop the main view so we can add a new output column
DROP VIEW public.v_clutch_instances;

-- 2) Recreate with corrected offspring codes and NEW full-token column
