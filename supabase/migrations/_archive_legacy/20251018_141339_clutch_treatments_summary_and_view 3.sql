...
select
    ...
    string_agg(distinct (material_type || ':' || material_code), ' ; ' order by material_type, material_code)
        as treatments_pretty,
    jsonb_agg(distinct jsonb_build_object(
        'type', material_type, 'code', material_code, 'name', material_name
    ) order by material_type, material_code) as treatments_json
from base  group by clutch_id;

-- Recreate the clutch overview view with treatments summary joined in
drop view if exists public.v_clutches_overview_final cascade;
