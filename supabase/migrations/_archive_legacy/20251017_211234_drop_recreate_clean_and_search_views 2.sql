-- Drop dependents, then drop & recreate v_fish_standard_clean with desired columns/order
...
fmt as (
    select
        ...
        -- clean genotype rollup (no background)
        trim(
            regexp_replace(
                concat_ws(
                    ' ',
...
select
    ...
    allele_nickname,
    allele_name,
    -- pretty strings computed here where allele_name is available
...
-- Recreate MV from the AS view, index for concurrent refresh
...
-- Refresh MV via helper if present (will now use CONCURRENTLY thanks to unique index)
do $$
