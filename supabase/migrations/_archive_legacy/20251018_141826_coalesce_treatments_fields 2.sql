...
-- (same CTEs mom/dad/core/joined as your current version) --
with mom as (
    select
        v.fish_code,
        string_agg(
...
select
    ...
    j.mom_strain,
    j.dad_strain,
    coalesce(s.treatments_count, 0)::int as treatments_count,
    j.clutch_birthday,
    j.date_planned,
    j.created_by_plan,
    j.created_at_plan,
    ...
    coalesce(s.treatments_pretty, '') as treatments_pretty,
    coalesce(s.treatments_json, '[]'::jsonb) as treatments_json
from joined AS j
