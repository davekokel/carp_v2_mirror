...
-- Roll up Tg(base)guN per fish (one row per fish_code)
...
-- Enriched clutch overview (no eggs/lay_date; your fields only)
...
select
  ...
  0::int                               as mom_n_living_tanks,     -- wire real counts later if needed
  ...
  0::int                               as dad_n_living_tanks,     -- wire real counts later if needed
