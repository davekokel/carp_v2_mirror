...
inner join public.fish_tank_memberships AS m
    on f.id = m.fish_id and m.left_at is null
inner join public.containers AS c
    on m.container_id = c.id
where
    c.status in ('active', 'new_tank')
...
from public.fish AS f
left join public.vw_fish_overview AS vo on f.fish_code = vo.fish_code
...
--    Pass your parsed tokens (genotype + strain) from the AS app.
...
as $$
with toks as (
  -- normalize and de-duplicate tokens from the AS app
