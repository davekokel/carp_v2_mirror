  ...
  SELECT
    ...
    CASE
      ...
      THEN format(
        ...
        ((regexp_match(regexp_replace(tr.tok, '[^a-z0-9\-]+', '', 'g'), '^([a-z]+)-?0*([0-9]+)$'))[2])::integer
      )
...
tg_entries AS (
  SELECT DISTINCT ON (gc.genotype_v11_id, gc.base_code_norm)
    ...
    gc.sort_num,
    CASE
      WHEN NULLIF(btrim(ta.allele_name), '') IS NOT NULL
       AND NULLIF(btrim(ta.allele_nickname), '') IS NOT NULL
        THEN format('%s-%s', btrim(ta.allele_name), btrim(ta.allele_nickname))
      WHEN NULLIF(btrim(ta.allele_name), '') IS NOT NULL
       AND ta.allele_number IS NOT NULL
        THEN format('%s-%s', btrim(ta.allele_name), ta.allele_number)
      WHEN NULLIF(btrim(ta.allele_nickname), '') IS NOT NULL
        THEN btrim(ta.allele_nickname)
      WHEN NULLIF(btrim(ta.allele_name), '') IS NOT NULL
        THEN btrim(ta.allele_name)
      WHEN NULLIF(btrim(ta.nickname), '') IS NOT NULL
        THEN btrim(ta.nickname)
      WHEN NULLIF(btrim(ta.display_name), '') IS NOT NULL
        THEN btrim(ta.display_name)
      ELSE NULL
