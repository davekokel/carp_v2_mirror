...
-- Adjust v11_treatment_star fluor_tag_label to use "fluor-tag(tag_pos)"
-- instead of "fluor::tag(tag_pos)". Org-style (fluor-organelle) and the
-- "no cytosol fallback" behavior stay as in the previous migration.
...
fusion_bits AS (
  SELECT
    ...
    --   no tag    -> fluor  (no invented ::cytosol)
    CASE
      WHEN tg.id IS NULL
