...
SELECT
  ...
  COALESCE(treats.treat_organelle_fluor, '') AS organelle_fluor_rollup,
  COALESCE(treats.treat_fluor_tag, '')       AS all_fluor_tag_rollup,
  COALESCE(treats.treat_organelle_fluor, '') AS all_organelle_fluor_rollup
FROM public.clutches c
LEFT JOIN imaging i
