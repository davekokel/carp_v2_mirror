...
immutable
as $$
declare v bigint := n; s text := ''; a constant text := '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
begin
  if v < 0 then raise exception 'to_base36 expects nonnegative'; end if;
  if v = 0 then return '0'; end if;
  while v > 0 loop s := substr(a, (v % 36) + 1, 1) || s; v := v / 36; end loop;
  return s;
end $$;
...
declare b text;
begin
  b := public.to_base36(nextval('public.fish_pair_b36_seq'));
...
do $$ begin
  if not exists (
    select 1 from pg_indexes where schemaname='public' and indexname='uq_fish_pairs_fish_pair_code'
  ) then
