...
treat_parts AS (
  SELECT
    ...
    -- TG-style token for treatment constructs
    ('tg(' || lower(coalesce(cons.base_code,'')) || ')') AS tok_tg,

    -- Fluor-tag style token for treatment constructs: fluor-tag(tagpos)
    CASE
    ...
    -- Fluor-organelle style token for treatment constructs: fluor-organelle
    CASE
...
SELECT
  ...
  -- TG style
  CASE
    WHEN coalesce(btrim(r.tx_gt_tg),'') <> '' AND r.tx_gt_tg LIKE '% > %'
      THEN r.tx_gt_tg
    WHEN coalesce(btrim(tt.treat_tg_typed),'') <> '' AND coalesce(btrim(r.tx_gt_tg),'') <> ''
      THEN tt.treat_tg_typed || ' > ' || r.tx_gt_tg
    WHEN coalesce(btrim(tt.treat_tg_typed),'') <> '' AND coalesce(btrim(r.tx_gt_tg),'') = ''
      THEN tt.treat_tg_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
    ELSE coalesce(r.tx_gt_tg,'')
  END AS tx_gt_tg,

  -- Fluor-tag style
  CASE
    WHEN coalesce(btrim(r.tx_gt_fluortag),'') <> '' AND r.tx_gt_fluortag LIKE '% > %'
      THEN r.tx_gt_fluortag
    WHEN coalesce(btrim(tt.treat_fluortag_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluortag),'') <> ''
      THEN tt.treat_fluortag_typed || ' > ' || r.tx_gt_fluortag
    WHEN coalesce(btrim(tt.treat_fluortag_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluortag),'') = ''
      THEN tt.treat_fluortag_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
    ELSE coalesce(r.tx_gt_fluortag,'')
  END AS tx_gt_fluortag,

  -- Fluor-organelle style
  CASE
    WHEN coalesce(btrim(r.tx_gt_fluororganelle),'') <> '' AND r.tx_gt_fluororganelle LIKE '% > %'
      THEN r.tx_gt_fluororganelle
    WHEN coalesce(btrim(tt.treat_fluororganelle_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluororganelle),'') <> ''
      THEN tt.treat_fluororganelle_typed || ' > ' || r.tx_gt_fluororganelle
    WHEN coalesce(btrim(tt.treat_fluororganelle_typed),'') <> '' AND coalesce(btrim(r.tx_gt_fluororganelle),'') = ''
      THEN tt.treat_fluororganelle_typed || CASE WHEN coalesce(btrim(c.genetic_background),'') <> '' THEN ' > ' || c.genetic_background ELSE '' END
