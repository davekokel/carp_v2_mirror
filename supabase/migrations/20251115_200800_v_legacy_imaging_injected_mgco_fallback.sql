...
SELECT
  ...
  CASE
    WHEN lip.plasmid_base_code IS NOT NULL THEN lip.plasmid_base_code
    WHEN ir.additional_plasmids_injected ~ '^MGCO-[0-9]+' THEN
      regexp_replace(ir.additional_plasmids_injected, '^(MGCO-[0-9]+).*$', '\1')
    WHEN ir.additional_plasmids_injected ~ '^MGCO[0-9]+' THEN
      'MGCO-' || regexp_replace(ir.additional_plasmids_injected, '^MGCO([0-9]+).*$', '\1')
    ELSE NULL
  END AS injected_plasmid_base_code,
