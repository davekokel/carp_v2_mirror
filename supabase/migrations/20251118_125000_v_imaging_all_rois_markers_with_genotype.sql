...
CREATE OR REPLACE VIEW public.v_imaging_all_rois_markers AS
-- 1) Mem-histone imaging rows (already include genotype markers + all_marker_fluor_codes)
SELECT
...
-- 2) Cluster-linked ROIs: join genotype markers and merge with treatment markers
SELECT
    ...
    -- unified marker fluors: genotype + treatment (inherited)
    TRIM(BOTH ',' FROM
        CONCAT(
            ...
            CASE WHEN gm.genotype_marker_fluor_codes IS NOT NULL
                     ...
                     AND r.eff_treatment_marker_fluor_codes IS NOT NULL
                     AND r.eff_treatment_marker_fluor_codes <> ''
                 THEN ',' ELSE '' END,
            COALESCE(r.eff_treatment_marker_fluor_codes, '')
        )
