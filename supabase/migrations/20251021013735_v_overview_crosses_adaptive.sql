...
begin
  if exists (
    ...
    execute $V$
      ...
      left join public.v_tanks_for_fish vt_m on vt_m.tank_id = ci.mother_tank_id
      left join public.v_tanks_for_fish vt_f on vt_f.tank_id = ci.father_tank_id;
    $V$;
  elsif exists (
    select 1 from information_schema.tables
    ...
    execute $V$
      ...
      left join public.v_tanks_for_fish vt_m on vt_m.tank_id = pc.mother_tank_id
      left join public.v_tanks_for_fish vt_f on vt_f.tank_id = pc.father_tank_id;
    $V$;
  else
    execute $V$
      create or replace view public.v_overview_crosses as
