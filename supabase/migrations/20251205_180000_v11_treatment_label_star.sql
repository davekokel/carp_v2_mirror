...
ingredient_kinds AS (
  -- subtypes: construct_kind (e.g. plasmid, rna, crispr, ...) + dye
  SELECT
...
SELECT
  ...
  -- label styles from v11_treatment_star
  j.all_fluor_tag_rollup        AS fluor_tag_style,
  j.all_organelle_fluor_rollup  AS fluor_organelle_style,

  -- treatment_display: {plasmid[MGCO-1+MGCO-4]; rna[MGCO-10]; dye}
  CASE
    WHEN COALESCE(j.materials_by_kind, '') = '' AND j.n_dyes = 0 THEN ''
    ELSE
      -- build base from materials_by_kind inside braces
      CASE
        WHEN COALESCE(j.materials_by_kind, '') = ''
          THEN '{'
        ELSE '{' || j.materials_by_kind
      END
      ||
      -- append "; dye" if dyes present and not already represented
      CASE
