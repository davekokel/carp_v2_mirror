...
SELECT
  ...
  COALESCE(fis.treatment_label_tg_style, fis.genotype_tg_style)
      AS treatment_or_genotype_tg_style,
  COALESCE(fis.treatment_label_fluortag_style, fis.genotype_fluortag_style)
      AS treatment_or_genotype_fluortag_style,
  COALESCE(fis.treatment_label_fluororganelle_style, fis.genotype_fluororganelle_style)
      AS treatment_or_genotype_fluororganelle_style,
  t.created_at
FROM public.tanks t
...
'Canonical tanks overview (v11): tanks + fish + line + genotype + treatment labels, including treatment>genotype vs genotype-only in tg/fluor-tag/fluor-organelle styles.';
