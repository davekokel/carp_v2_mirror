...
-- 1) Ensure uniqueness on (fish_id, transgene_base_code).
--    (Index name is stable but the function will NOT depend on it.)
CREATE UNIQUE INDEX IF NOT EXISTS ux_fta_fish_base
  ON public.fish_transgene_alleles (fish_id, transgene_base_code);

-- 2) Simplify the wrapper: conflict by column list, no named constraint,
--    and fully-qualify RETURNING to avoid OUT-param ambiguity.
...
BEGIN
  ...
  -- Link fish to that allele; if row exists for (fish_id, base), update number.
  RETURN QUERY
