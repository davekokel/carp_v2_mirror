...
SELECT
  COALESCE(x.id, x.id_uuid) AS id_uuid,
  x.mother_code,
  x.father_code,
  x.planned_for,
  x.created_by,
  x.created_at,
  CASE WHEN EXISTS (
    SELECT 1 FROM public.clutches c WHERE c.cross_id = COALESCE(x.id, x.id_uuid)
...
SELECT
  ...
  p.notes,
  p.created_at,
  COALESCE(r.id, r.id_uuid)    AS rna_id,
  r.code                       AS rna_code,
  r.name                       AS rna_name,
  r.source_plasmid_id
FROM public.plasmids p
LEFT JOIN public.rnas r ON r.source_plasmid_id = COALESCE(p.id, p.id_uuid)
ORDER BY p.code;
