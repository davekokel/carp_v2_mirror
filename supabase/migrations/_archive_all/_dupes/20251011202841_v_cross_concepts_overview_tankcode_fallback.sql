...
select
  ...
  -- mom tank: prefer prefilled mother_tank_id; else derive from live membership (tank_code OR label)
  coalesce(
    (select coalesce(c.tank_code, c.label) from public.containers c where c.id_uuid = pc.mother_tank_id),
  ...
  -- dad tank: prefer prefilled father_tank_id; else derive from live membership (tank_code OR label)
  coalesce(
    (select coalesce(c.tank_code, c.label) from public.containers c where c.id_uuid = pc.father_tank_id),
    (select coalesce(c2.tank_code, c2.label)
       from public.fish f2
       join public.fish_tank_memberships m2 on m2.fish_id=f2.id and m2.left_at is null
       join public.containers c2 on c2.id_uuid=m2.container_id and c2.status in ('active','new_tank')
