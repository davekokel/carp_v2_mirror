...
ci_tx as (
  select cit.clutch_instance_id::uuid as clutch_instance_id,
...
select
  ...
  -- planned placeholders (no concept store right now)
  null::text                                         as name,
  ...
  -- pretty helper
  (coalesce(p.mom_fish_code,'') || ' x ' || coalesce(p.dad_fish_code,'')) as cross_name_pretty
...
'Tank-centric clutch overview incl. cross_name_pretty (mom_code || '' x '' || dad_code).';
