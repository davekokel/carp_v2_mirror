...
-- Rebuild v_clutch_instances with corrected codes/tokens and both fusion lineages
...
-- Extract full Tg(...) tokens and inner plasmid codes (case-insensitive), with ordinals
geno_tokens AS (
  SELECT
    b.clutch_code,
    m.ord::int                                 AS ord,
...
-- Trim + distinct for tokens; codes distinct for genotype_fusions, but keep order
codes_distinct AS (
...
-- Offspring genotype (codes) ; joined (set-like, order by first seen)
geno_codes_join AS (
  SELECT clutch_code,
...
-- Offspring genotype (full tokens) ; joined
geno_tokens_join AS (
  SELECT clutch_code,
...
-- Fusions derived from codes (via plasmid_fusion_label) ; joined
geno_fusions AS (
  SELECT
    t.clutch_code,
...
SELECT
  ...
  -- Lineage (Tx fusions → Genotype codes)
  CASE
  ...
  -- Lineage (Tx fusions → Genotype fusions)
  CASE
    WHEN NULLIF(tf.clutch_treatments_fusions, '') IS NOT NULL
  ...
  -- Lineage (Tx codes → Genotype tokens)
  CASE
    WHEN NULLIF(tr.clutch_treatments_codes, '') IS NOT NULL
  ...
  -- ✅ NEW: Lineage (Tx codes → Genotype fusions)
  CASE
    WHEN NULLIF(tr.clutch_treatments_codes, '') IS NOT NULL
      THEN tr.clutch_treatments_codes || ' > ' || COALESCE(gr.clutch_genotype_fusions, '')
    ELSE COALESCE(gr.clutch_genotype_fusions, '')
  END AS clutch_lineage_codes_to_fusions_pretty
