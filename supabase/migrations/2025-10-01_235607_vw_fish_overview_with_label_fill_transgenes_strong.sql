-- Recreate labeled overview; compute *_filled by matching v.fish_code → fish → links inside each subquery
...
SELECT
  ...
  -- Prefer original columns; else pull first linked allele by matching fish_code in-subquery
  COALESCE(
    ...
    (
      ...
      WHERE UPPER(TRIM(f2.fish_code)) = UPPER(TRIM(v.fish_code))
...
-- keep this join only for batch label + created_by enrichment
