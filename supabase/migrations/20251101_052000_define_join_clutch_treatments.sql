...
-- Remove any legacy table if the baseline or old dumps reintroduced it
DROP TABLE IF EXISTS public.clutch_materials CASCADE;

-- Canonical join table: clutch -> treatments
...
-- Normalized keys for conflict/uniqueness (idempotent add)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Unique constraint for UI ON CONFLICT (idempotent add)
DO $$
BEGIN
  IF NOT EXISTS (
