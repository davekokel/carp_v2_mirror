...
per_base AS (
  SELECT
    ...
    allele_number,
    row_number() OVER (
      PARTITION BY fish_id, transgene_base_code
      ORDER BY allele_number DESC
    ) AS rn
  FROM allele_details
),
...
canonical AS (
  SELECT
    fish_id,
    transgene_base_code,
    allele_label AS canonical_label
  FROM per_base
  WHERE rn = 1
),
...
others AS (
  ...
  FROM per_base
  WHERE rn > 1
  GROUP BY fish_id, transgene_base_code
),

allele_priority_blocks AS (
  SELECT
    c.fish_id,
    c.transgene_base_code,
    CASE
      WHEN o.other_labels IS NULL THEN c.canonical_label
      ELSE c.canonical_label || ' > ' || o.other_labels
    END AS base_block
  FROM canonical AS c
  LEFT JOIN others AS o
    ON o.fish_id = c.fish_id
   AND o.transgene_base_code = c.transgene_base_code
),

allele_priority_labels AS (
  SELECT
    fish_id,
    string_agg(
      base_block,
      '; ' ORDER BY transgene_base_code
    ) AS genotype_alleles_priority_pretty
  FROM allele_priority_blocks
  GROUP BY fish_id
),
...
-- NEW: explicit RNA(basecode) labels for treatments
treatment_rna_codes_raw AS (
  SELECT
    ft.fish_id,
    'RNA(' || r.rna_base_code || ')' AS rna_label
  FROM public.join_fish_treatments AS ft
  JOIN public.join_treatment_rnas AS jtr
    ON jtr.treatment_id = ft.treatment_id
  JOIN public.rnas AS r
...
treatment_rna_codes AS (
  SELECT
    fish_id,
    string_agg(
      DISTINCT rna_label,
      ', ' ORDER BY rna_label
    ) AS treatment_rna_codes
  FROM treatment_rna_codes_raw
  GROUP BY fish_id
),
...
SELECT
  ...
  COALESCE(
    gl.genotype_label,
    apl.genotype_alleles_priority_pretty,
...
LEFT JOIN allele_priority_labels   AS apl ON apl.fish_id = f.id
...
LEFT JOIN treatment_rna_codes      AS trc ON trc.fish_id = f.id
...
SELECT
  ...
  fm.genotype_alleles_priority_pretty AS female_genotype_alleles_priority_pretty,
  fd.genotype_alleles_priority_pretty AS male_genotype_alleles_priority_pretty,
  (fm.genotype_alleles_priority_pretty || ' × ' || fd.genotype_alleles_priority_pretty)
    AS allele_priority_cross_label,
  ...
  fm.treatment_rna_codes AS female_treatment_rna_codes,
  fd.treatment_rna_codes AS male_treatment_rna_codes,
  (fm.treatment_rna_codes || ' × ' || fd.treatment_rna_codes)
    AS treatment_rna_cross_label,
