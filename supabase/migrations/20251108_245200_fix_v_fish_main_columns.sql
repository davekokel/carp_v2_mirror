...
-- Recreate v_fish_main with the columns the page selects.
-- Depends on: v_fluorescent_marker_rollup, v_fluorescent_marker_rollup_by_base, v_fish_unified.

CREATE OR REPLACE VIEW public.v_fish_main AS
SELECT
  f.fish_code,
  -- minimal set required by the page:
  ...
  COALESCE(
    ...
    jfta.transgene_base_code||'-'||LPAD(jfta.allele_number::text,2,'0')
  ) AS transgene_pretty_nickname,
  jfta.transgene_base_code AS transgene_pretty_name,
  u.genotype_pretty,
  r.fluors,                      -- fish-level (all bases)
  r.tags,                        -- fish-level (all bases)
  ''::text AS dyes               -- compatibility; fill when dyes table is ready
FROM public.fish f
