-- Enrich overview with transgene fallbacks (no transgene_name dependency)
...
tg_first AS (
  SELECT
    ...
    CASE WHEN array_length(allele_nums,1) > 0 THEN allele_num_first END AS allele_num_first
  FROM tg
)
SELECT
  v.*,
