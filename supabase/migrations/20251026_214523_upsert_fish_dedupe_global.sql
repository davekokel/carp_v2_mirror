-- 1) Make the batch→fish mapping idempotent
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes
     WHERE schemaname='public' AND indexname='uq_fsbm_seed_fish'
  ) THEN
    CREATE UNIQUE INDEX uq_fsbm_seed_fish
      ON public.fish_seed_batches_map (seed_batch_id, fish_uuid);
  END IF;
END$$;

-- 2) Replace upsert to de-dup by (seed_batch_id,name,dob) first;
--    if not found, re-use globally by (name_norm, dob, genetic_background)
...
DECLARE
  ...
  v_fc text;
BEGIN
  -- A) Try to find in THIS batch by (name,dob)
  ...
  -- B) If not found, try GLOBAL reuse by (name_norm, dob, genetic_background)
  ...
  -- C) If still not found, create a new fish
  IF v_fu IS NULL THEN
  ...
  -- D) Ensure a batch→fish mapping row exists (idempotent)
  INSERT INTO public.fish_seed_batches_map(fish_uuid, seed_batch_id)
  VALUES (v_fu, p_seed_batch_id)
  ON CONFLICT ON CONSTRAINT uq_fsbm_seed_fish DO NOTHING;
