...
treat_parts AS (
  SELECT DISTINCT
    oj.clutch_code,
    CASE
      WHEN lower(coalesce(tmc.delivery_form,'')) IN ('plasmid','dna') THEN 1
      WHEN lower(coalesce(tmc.delivery_form,'')) IN ('rna','mrna')     THEN 2
      WHEN lower(coalesce(tmc.delivery_form,'')) = 'crispr'           THEN 3
      ELSE 9
    END AS delivery_rank,
    CASE
      WHEN lower(coalesce(tmc.delivery_form,'')) IN ('plasmid','dna') THEN 'plasmid'
      WHEN lower(coalesce(tmc.delivery_form,'')) IN ('rna','mrna')     THEN 'rna'
      WHEN lower(coalesce(tmc.delivery_form,'')) = 'crispr'           THEN 'crispr'
      ELSE lower(coalesce(tmc.delivery_form,'unknown'))
    END AS delivery_form_norm,
    lower(coalesce(cons.base_code,'')) AS base_code_norm
  FROM one_jct oj
  LEFT JOIN public.treatment_mix_constructs tmc
    ON tmc.mix_id = oj.treatment_id
...
one_treat AS (
  SELECT
    clutch_code,
    string_agg(
      delivery_form_norm || '(' || base_code_norm || ')',
      '|' ORDER BY delivery_rank, delivery_form_norm, base_code_norm
