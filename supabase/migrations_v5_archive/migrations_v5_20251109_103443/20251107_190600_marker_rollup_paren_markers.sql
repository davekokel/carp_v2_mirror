...
SELECT
  ...
  string_agg(
    DISTINCT (
      b.transgene_base_code || '(' ||
      COALESCE(NULLIF(ta.allele_nickname,''), b.allele_number::text) || ')'
    ),
    ',' ORDER BY
      (b.transgene_base_code || '(' ||
       COALESCE(NULLIF(ta.allele_nickname,''), b.allele_number::text) || ')')
  ...
  ''::text AS dyes
FROM base b
LEFT JOIN fus x  ON x.fish_code=b.fish_code
                AND x.transgene_base_code=b.transgene_base_code
                AND x.allele_number=b.allele_number
LEFT JOIN public.fluors fl ON fl.id = x.fluor_id
LEFT JOIN public.tags   tg ON tg.id = x.tag_id
LEFT JOIN public.transgene_alleles ta
       ON ta.transgene_base_code=b.transgene_base_code
      AND ta.allele_number      =b.allele_number
