...
def _require_fish_name_only(df, filename: str):
    ...
    if "name" in cols:
        raise ValueError(
            f"{filename} must NOT include a 'name' column. Use only 'fish_name'."
        )
...
def _insert_fish_by_name(cx, df_fish: pd.DataFrame, default_batch: Optional[str]):
    ...
    _exec(
        cx, "ALTER TABLE public.fish ADD COLUMN IF NOT EXISTS line_building_stage text;"
    )
...
def _build_preview(df_fish, df_links, df_tg, default_batch):
    ...
    if _has_rows(df_links):
        ...
        g_tg = (
            ...
            .reset_index()
            .rename(columns={"tg_name": "transgenes"})
        )
        tg_df = g_tg
        ...
        g_alle = (
            ...
            .reset_index()
            .rename(columns={"alle_label": "alleles"})
        )
        alle_df = g_alle
...
batch_override = st.text_input(
    "Batch ID override (optional)",
    value="",
    placeholder="leave blank to use full zip filename",
)
...
if zip_file:
    ...
    if _has_rows(df_02):
        df_02 = _clean_df(df_02)
    if _has_rows(df_03):
        df_03 = _clean_df(df_03)
    if _has_rows(df_01):
        df_01 = _clean_df(df_01)
    if _has_rows(df_10):
        df_10 = _clean_df(df_10)

    # Strict: fish & links must have fish_name (and NOT 'name')
    if df_01 is not None:
        _require_fish_name_only(df_01, "01_fish.csv")
    if df_10 is not None:
        _require_fish_name_only(df_10, "10_fish_transgene_alleles.csv")

    derived_batch = batch_override.strip() or _derive_batch_id_from_zip(zip_file.name)
    st.success(
        f"Derived batch_id → **{derived_batch}** (this will be written to `fish.batch_label`)"
    )
                ...
                if _has_rows(df_10):
                    for c in [
                        ...
                        if c not in df_10.columns:
                            df_10[c] = None
                    base_codes = sorted(
                        df_10["transgene_base_code"].dropna().unique().tolist()
                    )
