...
# force PyPI supabase client (avoid local ./supabase package)
...
if user:
    st.success(f"Signed in as {getattr(user, "email", None)}")
    st.stop()

st.title("Sign in — 6-digit code")
...
with c1: st.button("Send code", on_click=send, use_container_width=True)
with c2: st.text_input("Enter 6-digit code", key="otp_code", max_chars=8)
with c3: st.button("Verify", on_click=verify, type="primary", use_container_width=True)
