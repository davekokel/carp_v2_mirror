...
def load_slot_ids(engine: Engine) -> Dict[Tuple[str, int], str]:
    """
    ...
    for slot_id, plate_code, slot_index in rows:
        if plate_code is None or slot_index is None:
            continue
        key = (str(plate_code), int(slot_index))
        mapping[key] = slot_id
    return mapping
...
def main() -> None:
    ...
    with engine.begin() as cx:
        for _, row in grouped.iterrows():
            ...
            if slot_id is None:
                print(
                    f"[WARN] No imaging_slot for plate='{plate_code}', slot_index={slot_index}; "
