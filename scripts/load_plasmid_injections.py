...
insert into public.injected_plasmid_treatments (fish_id, plasmid_id, amount, units, note)
values (:fish_id, :plasmid_id, :amount, :units, :note)
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Load injected plasmid treatments from a narrow CSV.")
    ap.add_argument("--db", required=True, help="DB URL (e.g., postgresql://user:pw@host:port/db?sslmode=require)")
    ap.add_argument("--csv", required=True, help="CSV with columns: fish_name, plasmid_code, amount, units, note")
    ...
    with eng.connect() as cx:
        ...
        try:
            for row in rows:
                ...
                plasmid_code = _str(row.get("plasmid_code"))
                if not fish_name or not plasmid_code:
                    continue
                ...
                pid = cx.execute(
                    ...
                    {"code": plasmid_code, "name": plasmid_code},
                ).scalar()
                if not pid:
                    continue
                ...
                res = cx.execute(
                    ...
                    {"fish_id": fish_id, "plasmid_id": pid, "amount": amount, "units": units, "note": note},
                )
    ...
    print(f"Inserted {inserted}/{attempted} plasmid treatment rows; {dupes} duplicates skipped; {skipped_missing_fish} rows skipped (fish not found).")
