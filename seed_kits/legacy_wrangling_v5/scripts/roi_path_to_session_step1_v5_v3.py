...
OUT_AMBIG = QC / "roi_path_to_session_step1_v5.qc_ambiguous.tsv"
OUT_MISS = QC / "roi_path_to_session_step1_v5.qc_missing.tsv"
...
def main() -> None:
    ...
    for _, r in roi.iterrows():
        ...
        if len(cand) == 0:
            out_rows.append({"roi_path": roi_path, "session_id": ""})
            miss.append({"roi_path": roi_path, "foundation_root": fr, "experiment_folder": ef, "reason": "no_candidates"})
            continue

        sess = sorted(set(cand["session_id"].astype(str).tolist()))
        if len(sess) == 1:
            out_rows.append({"roi_path": roi_path, "session_id": sess[0]})
            continue

        toks = roi_clue_tokens(roi_path)
        if not toks:
            out_rows.append({"roi_path": roi_path, "session_id": ""})
            ambig.append({"roi_path": roi_path, "foundation_root": fr, "experiment_folder": ef, "reason": "multi_sessions_no_tokens", "sessions": " || ".join(sess)})
            continue
        ...
        if top_score == 0:
            out_rows.append({"roi_path": roi_path, "session_id": ""})
            ambig.append({"roi_path": roi_path, "foundation_root": fr, "experiment_folder": ef, "reason": "no_token_hits", "sessions": " || ".join(sess), "tokens": ";".join(toks)})
            continue

        if len(top) == 1:
            out_rows.append({"roi_path": roi_path, "session_id": str(top.iloc[0]["session_id"])})
            continue
        ...
        ambig.append({
            ...
            "reason": "tie_on_score",
            "top_score": top_score,
            "sessions": " || ".join(sorted(set(top["session_id"].astype(str).tolist()))),
            "tokens": ";".join(toks),
        })
