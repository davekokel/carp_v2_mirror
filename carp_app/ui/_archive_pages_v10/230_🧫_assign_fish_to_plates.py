...
# 🧫 Plates — assign clutches & orientations (v11 imaging schema)
...
st.set_page_config(
    page_title="🧫 Plates — assign clutches & orientations",
    page_icon="🧫",
    layout="wide",
)
st.title("🧫 Plates — assign clutches & orientations")
...
if st.button("➕ Create new imaging plate", type="primary", use_container_width=True):
    ...
    st.session_state["_assign_plate_id"] = plate_id
    st.session_state["_assign_plate_code"] = plate_code
    st.success(
        f"Created imaging plate {plate_code} ({microscope_type_sel}) with {len(slots)} slots."
    )
