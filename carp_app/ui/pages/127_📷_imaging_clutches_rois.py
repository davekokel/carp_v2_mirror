...
with st.form("roi_filters", clear_on_submit=False):
    ...
    with c1:
        q_raw = st.text_input(
            "Search (plate / slot / fish / ROI / genotype)",
            "",
            help=(
...
marker_q = _normalize_q(marker_q_raw)
...
sql = text(
    ...
    WHERE
      ...
      AND (
        :marker_q IS NULL
        OR COALESCE(all_marker_fluor_codes,'') ILIKE :marker_like
      )
    ORDER BY
...
params = {
    ...
    "marker_q": marker_q,
    "marker_like": f"%{marker_q}%" if marker_q else None,
    "lim": lim,
}
