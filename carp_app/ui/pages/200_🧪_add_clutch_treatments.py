...
def _insert_instance_treatments(clutch_instance_id: str, treated_clutch_id: str, created_by: str, items: List[Dict]):
    ...
    with _eng().begin() as cx:
        for it in items:
            kind = str(it.get("treatment_type") or "").strip()    # 'plasmid'|'rna'|'dye'|'crispr'
            code = str(it.get("code") or "").strip()
            name = str(it.get("name") or "").strip()
            if not kind or not code:
                errs.append("<missing kind/code> → skipped"); continue

            # find or create treatment
            tid = pd.read_sql(
                text("SELECT id::text AS id FROM public.treatments WHERE kind_code=:k AND treat_code=:c LIMIT 1"),
                cx, params={"k": kind, "c": code}
            )
            if tid.empty:
                tid = pd.read_sql(
                    text("""
                    ...
                    """),
                    cx, params={"k": kind, "c": code, "name": name, "who": created_by or ""}
                )
            treatment_id = tid["id"].iloc[0]

            # link (only required columns in join table)
            exists = pd.read_sql(
                text("""
                  SELECT 1
                  ...
                  LIMIT 1
                """),
                cx, params={"gid": treated_clutch_id, "tid": treatment_id}
            )
            if exists.empty:
                cx.execute(
                    text("""
...
if r.empty:
    st.error("Could not resolve cross_instance_id for the selected clutch."); st.stop()
...
with tabs[1]:
    c1, c2 = st.columns([2,1])
    with c1: q_rna = st.text_input("Search RNAs (code / nickname / notes)", value="")
    ...
    if df_rna.empty:
        ...
        sel_mask_rna = eg_rna.get("✓ Select").fillna(False).astype(bool) if "✓ Select" in df_rna.columns else pd.Series([], dtype=bool)
...
with tabs[2]:
    ...
    if df_dye.empty:
        ...
        sel_mask_dye = eg_dye.get("✓ Select").fillna(False).astype(bool) if "✓ Select" in df_dye.columns else pd.Series([], dtype=bool)
...
with tabs[3]:
    c1, c2 = st.columns([2,1])
    with c1: q_cr = st.text_input("Search CRISPR knockins (code / description)", value="")
    with c2: note_cr = st.text_input("Note for selected knockins", value="")
    ...
    if df_cr.empty:
        ...
        sel_mask_cr = eg_cr.get("✓ Select").fillna(False).astype(bool) if "✓ Select" in df_cr.columns else pd.Series([], dtype=bool)
...
grp_sql = text("""
  ...
  SELECT
    ...
    ''::text AS treatment_genotype_group
  FROM s
