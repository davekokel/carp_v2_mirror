...
st.set_page_config(page_title="CARP — Overview: Tags", page_icon="🔎", layout="wide")
...
with st.form("filters", clear_on_submit=False):
    ...
    with c1:
        q_raw = st.text_input("Search (code / name / localization / note / citation link)", "")
    with c2:
...
sql = text("""
  ...
  WHERE (
    ...
    OR COALESCE(note,'')          ILIKE :ql
    OR COALESCE(citation_link,'') ILIKE :ql
  )
...
with col_left:
    ...
    editable_cols = st.multiselect(
        "Editable columns",
        ["code", "name", "localization", "note", "citation_link"],
        default=[],
    )
with col_right:
...
if not selected_ids or not editable_cols:
    ...
    colcfg = {
        "id":             st.column_config.TextColumn("ID", disabled=True),
        "code":           st.column_config.TextColumn("Code", disabled=("code" not in editable_cols)),
        ...
        "note":           st.column_config.TextColumn("Note", disabled=("note" not in editable_cols)),
        "citation_link":  st.column_config.LinkColumn("Citation link", disabled=("citation_link" not in editable_cols)),
    }

    st.caption("3) Edit the selected rows (changes are not persisted; export to CSV below)")
