# carp_app/ui/pages/121_🔎_overview_fluors.py
...
st.set_page_config(page_title="CARP — Overview: Fluors", page_icon="🔎", layout="wide")
st.title("🔎 Overview: Fluors")
...
with st.form("filters", clear_on_submit=False):
    ...
    with c1:
        q_raw = st.text_input("Search (code / name / alt_names / notes)", "")
    with c2:
...
with col_left:
    ...
    editable_cols = st.multiselect(
        "Editable columns",
        ["code", "name", "excitation_nm", "emission_nm", "alt_names", "notes"],
        default=[],
    )
with col_right:
...
if not selected_ids or not editable_cols:
    ...
    st.download_button(
        ...
        file_name=f"fluors_edited_{len(edited)}_{('all' if not q else q)}.csv",
        type="primary",
        mime="text/csv",
    )
...
st.download_button(
    ...
    file_name=f"fluors_overview_{lim}_{('all' if not q else q)}.csv",
    type="secondary",
    mime="text/csv",
)