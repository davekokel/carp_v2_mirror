...
st.set_page_config(page_title="CARP — 🐣 Annotate Treated Clutch Groups",
...
with st.form("filters", clear_on_submit=False):
    ...
    with c1: d_from = st.date_input("From", value=today - timedelta(days=120))
    with c2: d_to   = st.date_input("To",   value=today + timedelta(days=14))
    with c3: qtxt   = st.text_input("Search (group/clutch/cross/genotype/treatments/parents)", value="")
...
def _require_objects():
    miss = []
    if not _exists_view(V_TCL_OV): miss.append(V_TCL_OV)
    for t in (T_TCL, T_JANN, T_ANN):
        if not _exists_table(t): miss.append(t)
    if miss:
        st.error("Required object not found: " + ", ".join(miss)); st.stop()
...
def _load_groups_filtered(d1: date, d2: date, q: str, most_recent: bool) -> pd.DataFrame:
    ...
    if not _exists_view(V_TCL_OV):
        st.error(f"Required view not found: {V_TCL_OV}"); st.stop()
    where, p = [], {}
    if not most_recent:
        where.append("group_created_at::date BETWEEN :d1 AND :d2"); p.update({"d1": d1, "d2": d2})
    if q.strip():
        p["q"] = f"%{q.strip()}%"
        where.append("""(
          treated_clutch_code ILIKE :q OR clutch_code ILIKE :q OR cross_code ILIKE :q OR
    ...
    sql = text(f"SELECT * FROM {V_TCL_OV} {wsql} ORDER BY clutch_code, group_created_at LIMIT 500")
    with eng.begin() as cx:
        df = pd.read_sql(sql, cx, params=p)

    # derive pretties expected by the existing grid (if you kept them)
    df["clutch_birthday"] = pd.NaT
    df["cross_name_pretty"] = df["cross_code"]
    df["clutch_genotype_pretty"] = df.get("clutch_genotype","").fillna("")
...
if picked.empty:
    st.info("Select a treated clutch group to annotate it."); st.stop()
...
def _resolve_ids_from_group(group_code: str) -> tuple[str|None, str|None]:
    sql = text("""
      SELECT tc.id::text AS treated_clutch_id, tc.clutch_instance_id::text AS clutch_instance_id
...
if not treated_clutch_id:
    st.error("Could not resolve treated_clutch_id for this group."); st.stop()
        ...
        sql = text("""
          WITH ranked AS (
            SELECT
              ...
              ROW_NUMBER() OVER (PARTITION BY ja.kind_code ORDER BY ja.created_at DESC NULLS LAST) rn
...
with c3:
    note_txt = st.text_input("notes", value=str(current.get("notes") or ""), placeholder="optional")
...
def _save_annotation_num(tid: str, kind: str, val: int, who: str):
    with eng.begin() as cx:
        cx.execute(text("""
          INSERT INTO public.join_annotations (target_kind, target_id, kind_code, value_num, created_at)
...
def _save_annotation_text(tid: str, kind: str, txt: str, who: str):
    ...
    with eng.begin() as cx:
        cx.execute(text("""
          INSERT INTO public.join_annotations (target_kind, target_id, kind_code, value_text, created_at)
          VALUES ('treated_clutch', CAST(:tid AS uuid), :k, NULLIF(:v,''), now())
        """), {"tid": tid, "k": kind, "v": txt})

if st.button("💾 Save", use_container_width=True):
    who = (getattr(user, "email", None) or os.getenv("USER") or os.getenv("USERNAME") or "system")
