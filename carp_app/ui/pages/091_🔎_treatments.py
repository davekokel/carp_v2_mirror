...
if df.empty:
    ...
    # keep an untouched copy to detect nickname changes
    df_orig = df.copy()

    # we don't show display_name in the main grid (mostly redundant now)
    view = pd.DataFrame(
        {
    ...
    # editable nickname; other fields read-only
    grid = st.data_editor(
        view,
        ...
        column_config={
            ...
            "treatment_id": st.column_config.TextColumn("treatment_id", disabled=True),
    ...
    # ════════════════════════════════════════════════════
    # detect nickname changes vs original
    ...
    if "✓ Select" in grid.columns:
        selected_idxs = (
            grid.index[grid["✓ Select"] == True].to_series().tolist()
        )
                    ...
                    text(
                        ...
                        construct_ing AS (
                          SELECT
                            ...
                            c.code          AS ingredient_code,
                            c.display_name  AS ingredient_name,
                            tmc.concentration
                          FROM mix_base mb
                        ...
                        dye_ing AS (
                          SELECT
                            ...
                            d.nickname     AS ingredient_code,
                            d.display_name AS ingredient_name,
                            tmd.concentration
                          FROM mix_base mb
                        ...
                        SELECT *
                        FROM dye_ing
                        ORDER BY mix_code, ingredient_type, ingredient_code
                        """
                    ),
                    cx,
                    params={"tid": treatment_id},
