...
cx_picker = st.data_editor(
    ...
    column_config={
        ...
        "mom_genotype_pretty": st.column_config.TextColumn(
            "Mom genotype", disabled=True, width="large"
        ),
        "dad_genotype_pretty": st.column_config.TextColumn(
            "Dad genotype", disabled=True, width="large"
        ),
    },
    key="cross_picker_editor_v11",
)
cx_mask = (
    cx_picker.get(cx_sel, pd.Series(False, index=cx_picker.index))
    .fillna(False)
    .astype(bool)
)
...
cl_mask = (
    cl_picker.get(cl_sel, pd.Series(False, index=cl_picker.index))
    .fillna(False)
    .astype(bool)
)
...
if not chosen_clutches.empty:
    cl_for_cross = chosen_clutches[
        chosen_clutches["cross_id"] == cx_row["cross_id"]
    ]
else:
...
if cl_for_cross.empty:
    st.info(
        "No selected clutches for this cross (choose some in Step 2 to preview clutch labels)."
    )
else:
    ...
    canonical_line = "; ".join(
        x
        for x in [
            cx_row.get("mom_allele_canonical", ""),
            cx_row.get("dad_allele_canonical", ""),
        ]
        if x
    )
    clutch_org_line = "; ".join(
        x
        for x in [
            cx_row.get("mom_organelle_fluor_rollup", ""),
            cx_row.get("dad_organelle_fluor_rollup", ""),
        ]
        if x
    )
    ...
    clutch_rows_preview: List[Tuple[str, str]] = [
        ...
        (
            "Line 1",
            f"{cx_row.get('cross_code','')} @ {cx_row.get('cross_date','')}",
        ),
...
for r in chosen_crosses.to_dict(orient="records"):
    ...
    allele_nick_row = (
        f"{r.get('mom_allele_nicknames','')} × {r.get('dad_allele_nicknames','')}".strip()
    )
    org_row = (
        f"{r.get('mom_organelle_fluor_rollup','')} × {r.get('dad_organelle_fluor_rollup','')}".strip()
    )
...
for r in chosen_clutches.to_dict(orient="records"):
    ...
    canonical_line = "; ".join(
        x
        for x in [
            cx.get("mom_allele_canonical", ""),
            cx.get("dad_allele_canonical", ""),
        ]
        if x
    )
    clutch_org_line = "; ".join(
        x
        for x in [
            cx.get("mom_organelle_fluor_rollup", ""),
            cx.get("dad_organelle_fluor_rollup", ""),
        ]
        if x
    )
    ...
    petri_rows.append(
        {
            "clutch_instance_code": cross_line,
            "clutch_name": "",
            "mom_code": "",
            "dad_code": "",
            "clutch_genotype": canonical_line,
            "date_birth": str(r.get("clutch_date") or ""),
            "tx_codes": "",
            "tx_fluors": clutch_org_line,
        }
    )
...
with c1:
    if HAVE_PRINT_HELPER:
        ...
        st.button(
            "🖨️ Print / Download CROSS labels (ALL selected)", disabled=True
        )
...
with c2:
    if HAVE_PRINT_HELPER:
        ...
        st.button(
            "🖨️ Print / Download CLUTCH labels (ALL selected)", disabled=True
        )