...
# Accept deep-link query params if available (Streamlit ≥1.30)
try:
    q = st.query_params
    ...
    if "date"   in q:
        try:
            WB["date"] = date.fromisoformat(q["date"])
        except Exception:
            pass
except Exception:
    pass
...
with c1:
    WB["clutch_code"] = st.text_input("Clutch code", value=WB["clutch_code"], placeholder="CL-…", key="chip_clutch")
with c2:
    WB["run_code"] = st.text_input("Run code", value=WB["run_code"], placeholder="XR-…", key="chip_run")
with c3:
    WB["date"] = st.date_input("Date", value=WB["date"], key="chip_date")
with c4:
    WB["created_by"] = st.text_input("Created by", value=WB["created_by"], key="chip_by")
...
def _goto(rel_page_path: str, **state):
    ...
    try:
        st.switch_page(rel_page_path)  # path relative to the Streamlit app root
    except Exception:
...
with tab_plan:
    ...
    with cpa:
        if st.button("➡️ Enter new clutches", use_container_width=True, key="plan_enter_new_clutches"):
            _goto("pages/030_🐟_enter_new_clutches.py")
    with cpb:
        if st.button("➡️ Enter new crosses", use_container_width=True, key="plan_enter_new_crosses"):
    ...
    with colA:
        if st.button("🔎 Overview — Crosses", use_container_width=True, key="plan_overview_crosses"):
            _goto("pages/023_🔎_overview_crosses.py")
    with colB:
        if st.button("🔎 Overview — Clutches", use_container_width=True, key="plan_overview_clutches"):
...
with tab_run:
    ...
    with c1:
        if st.button("➡️ Enter cross instance (schedule & labels)", use_container_width=True, key="run_enter_cross_instance"):
            _goto(
                "pages/032_🐟_enter_cross_instance.py",
    ...
    if st.button("📊 Daily Overviews (All)", use_container_width=True, key="run_daily_overviews"):
...
with tab_annotate:
    ...
    with ca:
        if st.button("➡️ Annotate clutch instances", use_container_width=True, key="annotate_open"):
            prefill = {}
            if WB["run_code"]:
...
with tab_review:
    ...
    with ra:
        if st.button("🔎 Overview — Crosses", use_container_width=True, key="review_overview_crosses"):
            _goto("pages/023_🔎_overview_crosses.py")
    with rb:
        if st.button("🔎 Overview — Clutches", use_container_width=True, key="review_overview_clutches"):
            _goto("pages/024_🔎_Overview_Clutches.py")
    with rc:
        if st.button("📊 Daily Overviews (All)", use_container_width=True, key="review_daily_overviews"):
            _goto("pages/026_🔎_daily_overview.py")
