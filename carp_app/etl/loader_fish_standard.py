...
def load_fish_standard_from_excel(xlsx_path: str | Path, engine: Optional[Engine] = None) -> dict:
    ...
    sql_lookup = text(
        ...
        WHERE birthday = :birthday
          AND COALESCE(genetic_background,'') = COALESCE(:bg,'')
          AND COALESCE(line_building_stage,'') = COALESCE(:stage,'')
          AND COALESCE(nickname,'') = COALESCE(:nick,'')
    ...
    with engine.begin() as cx:
        for _, row in df.iterrows():
            ...
            existing = cx.execute(sql_lookup, params_key).scalar()
            if existing:
                continue
